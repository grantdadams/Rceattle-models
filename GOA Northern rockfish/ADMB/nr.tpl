...
// Read data from the control file
  !! ad_comm::change_datafile_name("goa_nr_2022.ctl");    //Read in phases, penalties and priors from "tem.ctl"
...
// Priors
  ...
  init_int    ph_ESS                // Phase for estimating effective sample size
  init_int    yrs_ESS               // Toggle for estimating ESS annualy (2) or as a mean (1)
  init_int    pen_ESS               // Toggle for including constraint funtion for ESS (0) no constraint (1) with constraint
  init_int    C_like_type           // Likelihood structure for catch biomass (1) normalized (2) full
  init_int    S_like_type           // Likelihood structure for survey biomass (1) normalized (2) full
...
// Survey age composition data
  ...
  init_vector   nmulti_srv1_age(1,nyrs_srv1_age)          // multinomial sample size
...
// Multinomial sample size calculations

// Hybrid method (Square root of hauls * totals) scaled to max 100
...
// Starting values for estimated parameters; these values over-ride all others
    ...
    a50_srv2                7.3
    delta_srv2              3.8
    Est_nsamples_fish_age   50
    Est_nsamples_srv1_age   50
    Est_nsamples_fish_size  50
    mat_a50                 9
    mat_delta               1
...
PARAMETER_SECTION
 ...
 // Stock-recruitment
  ...
  vector              log_fish_sel(1,nages_M);                    // vector of fishery selectivy log parameters including those not estimated
 ...
 // Trawl Survey selectivity
  ...
  vector              log_srv1_sel(1,nages_M);                    // vector of survey selectivy log parameters including those not estimated
 ...
 // Longline Survey selectivity
  ...
  vector              log_srv2_sel(1,nages_M);                    // vector of survey selectivity log parameters including those not estimated
  vector              srv2_sel(1,nages_M);                        // vector of survey selectivity parameters on arithmetic scale
 ...
 // Effective sample size parameters
  init_bounded_number Est_nsamples_fish_age(10,250,ph_ESS)
  init_bounded_number Est_nsamples_srv1_age(10,250,ph_ESS)
  init_bounded_number Est_nsamples_fish_size(10,250,ph_ESS)

 //Parameters to estimate maturity

  init_number mat_a50(2)
  init_number mat_delta(2)

  vector L_pmat(1,nages_mat)
  vector C_pmat(1,nages_mat)
  vector Pred_pmat(1,nages_mat)
  vector   p_mature(1,nages_M)
  vector wt_mature(1,nages_M);                  // Weight of mature fish vector at age


  vector Like_L_vec(1,nages_mat)
  vector Like_C_vec(1,nages_mat)
...
// Likelihoods and penalty functions
  ...
  matrix              dirich_fish_age(1,nyrs_fish_age,1,nages_D)
  matrix              dirich_srv1_age(1,nyrs_srv1_age,1,nages_D)
  matrix              dirich_fish_size(1,nyrs_fish_size,1,nlenbins)
  matrix              dir_fish_age_pen(1,nyrs_fish_age,1,nages_D)
  vector              dir_fa_pen(1,nyrs_fish_age)
  matrix              dir_srv1_age_pen(1,nyrs_srv1_age,1,nages_D)
  vector              dir_sa_pen(1,nyrs_srv1_age)
  matrix              dir_fish_size_pen(1,nyrs_fish_size,1,nlenbins)
  vector              dir_fs_pen(1,nyrs_fish_size)
  matrix              digamma_fish_age_n(1,nyrs_fish_age,1,nages_D)
  matrix              digamma_fish_eac(1,nyrs_fish_age,1,nages_D)
  matrix              digamma_srv1_age_n(1,nyrs_srv1_age,1,nages_D)
  matrix              digamma_srv1_eac(1,nyrs_srv1_age,1,nages_D)
  matrix              digamma_fish_size_n(1,nyrs_fish_size,1,nlenbins)
  matrix              digamma_fish_esc(1,nyrs_fish_size,1,nlenbins)
