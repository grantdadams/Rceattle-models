# Code used to run the 2023 arrowtooth flounder comparison between ADMB and CEATTLE
...
SAFE2023_mod$quantities$srv_bio_hat <- SAFE2023_index %>%
  dplyr::filter(Year %in% SAFE2023_mod$data_list$index_data$Year) %>%
  dplyr::pull(Pred)


# - Index model as biomass
index_mod <- ceattle_ss
yrs_srv <- ceattle_ss$data_list$index_data$Year - 1977 + 1
index_mod$quantities$biomass[1,] <- 1
index_mod$quantities$biomass[1,yrs_srv] <- ceattle_ss$data_list$index_data$Observation
