...
weighted_ration <- function(Rceattle, spp = 1, endyr = 2023, minage = 4, maxage = max(Rceattle$data_list$nages)){
  ...
  return(apply(Rceattle$quantities$ration[spp,,minage:maxage,1:length(yrs)] * Rceattle$quantities$biomass_at_age[spp,,minage:maxage,1:length(yrs)], 3, sum))
}
...
ESR_plot_M_age <-function(msModel, ssModel, age = 1, endyr = 2023){
  ...
  M2 <- rbind(
    data.frame(Year = c(yrs, 2050), SSM= c(ssModel$quantities$M_at_age[1,1,age,1:length(yrs)], 1) , MSM= c(msModel$quantities$M_at_age[1,1,age,1:length(yrs)], NA), Species = "a) Prey: Walleye pollock"),
    data.frame(Year = c(yrs, 2050),SSM= c(ssModel$quantities$M_at_age[3,1,age,1:length(yrs)], .4) , MSM= c(msModel$quantities$M_at_age[3,1,age,1:length(yrs)], NA), Species = "b) Prey: Pacific cod"),
    data.frame(Year = c(yrs, 2050),SSM= c(ssModel$quantities$M_at_age[2,1,age,1:length(yrs)], 0.15) , MSM= c(msModel$quantities$M_at_age[2,1,age,1:length(yrs)], NA), Species = "c) Prey: Arrowtooth (F)"),
    data.frame(Year = c(yrs, 2050),SSM= c(ssModel$quantities$M_at_age[2,2,age,1:length(yrs)], 0.3) , MSM= c(msModel$quantities$M_at_age[2,2,age,1:length(yrs)], NA), Species = "d) Prey: Arrowtooth (M)"))
...
ESR_plot_propM<-function(Rceattle, age = 1, species = 1, sex = 1, endyr = 2023){
  ...
  yrs <- Rceattle$data_list$styr:endyr
  M2age <- Rceattle$quantities$M2_at_age[species,sex,age,1:length(yrs)]
  nspp <- Rceattle$data_list$nspp
  
  M2_pred_pollock  <- colSums(Rceattle$quantities$M2_at_age[1 + (nspp * 0), species + (nspp * (sex-1)),,age,1:length(yrs)])
