...
years <- ceattle_esr_model$data_list$styr:2025
...
M_at_age_sd <- replace(M_at_age, values = ceattle_esr_model$sdrep$sd[which(names(ceattle_esr_model$sdrep$value) == "M_at_age")])
...
for(sp in 1:3){
  for(sex in 1:ceattle_esr_model$data_list$nsex[sp]){
    ...
    # Sex-specific M_at_age
    peak_M[ind] <- max(ceattle_esr_model$quantities$M_at_age[sp,sex,1,1:length(years)])
    avg_M[ind] <- mean(ceattle_esr_model$quantities$M_at_age[sp,sex,1,1:length(years)])
    terminal_M[ind] <- ceattle_esr_model$quantities$M_at_age[sp,sex,1,length(years)]
    ...
    peak_M_ind[ind] <- which(ceattle_esr_model$quantities$M_at_age[sp,sex,1,1:length(years)] == peak_M[ind])
...
for(i in c(1,3)){
  ...
  Matage1 = data.frame(Year = years,
                       MS_M = mod_list_all[[3]]$quantities$M_at_age[sp,1,1,1:length(years)], 
                       SS_M = mod_list_all[[1]]$quantities$M_at_age[sp,1,1,1:length(years)])
  
  Biomass = data.frame(Year = years,
                       SS_Biomass = mod_list_all[[1]]$quantities$biomass[sp,1:length(years)],
                       SS_SSB = mod_list_all[[1]]$quantities$ssb[sp,1:length(years)],
                       
                       MS_Biomass = mod_list_all[[3]]$quantities$biomass[sp,1:length(years)],
                       MS_SSB = mod_list_all[[3]]$quantities$ssb[sp,1:length(years)]
  )
  ...
  Ration = data.frame(Year = years,
                      `Age4plusTotal` = weighted_ration(ceattle_esr_model, spp = sp, endyr = 2025, minage = 4),
                      `Age1plusTotal` =weighted_ration(ceattle_esr_model, spp = sp, endyr = 2025, minage = 1)
  )
  ...
  write_xlsx(
    ...
    ,paste0("Results/GOA 2025 ", spname, " ESP Indices.xlsx")
    )
}
...
Female_M = cbind(years,t(mod_list_all[[3]]$quantities$M_at_age[2,1,,1:length(years)]), data.frame(M = mod_list_all[[1]]$quantities$M_at_age[2,1,1,1:length(years)]))

colnames(Female_M) <- c("Year", paste0("Multi-spp M Age_",1:nages), "Single-spp M")

Male_M = cbind(years,t(mod_list_all[[3]]$quantities$M_at_age[2,2,,1:length(years)]), data.frame(M = mod_list_all[[1]]$quantities$M_at_age[2,2,1,1:length(years)]))
...
write_xlsx(
  list(Female_M = Female_M,
       ...
       Biomass = data.frame(Year = years,
                            ...
                            SS_SSB = mod_list_all[[1]]$quantities$ssb[2,1:length(years)],
                            
                            MS_Biomass = mod_list_all[[3]]$quantities$biomass[2,1:length(years)],
                            MS_SSB = mod_list_all[[3]]$quantities$ssb[2,1:length(years)]
       ),
  ...
  ,"Results/GOA 2025 ATF ESP Indices.xlsx")
