---
title: 2025 Climate-enhanced multispecies stock assessment for walleye pollock, Pacific cod, and arrowtooth flounder in the Gulf of Alaska
author: 
  - "Grant Adams,  Kirstin K. Holsman, Kalei Shotwell" 
...
# write mortality to files for ESP
for(mode in c("Multispecies","Singlespecies")){
  M2 <- list()
  for(s in 1:3){
    eval(parse(text = paste0("M2_tmp <- ceattle_2$M2_",s," %>%data.frame()")))
    M2[[spnames[s]]]           <- M2_tmp
    colnames(M2[[spnames[s]]]) <- paste("Age",1:nages[s])
    
    M2[[spnames[s]]] <- M2[[spnames[s]]]%>%
      mutate(model = mode,type="M2",species = spnames[s], year = years)%>%
       relocate(model,species,type, year)
  }
  
  
  M1 <- list()
  for(s in 1:3){
    eval(parse(text = paste0("M1_tmp <- ceattle_2$M1_",s," %>%data.frame()")))
    M1[[spnames[s]]]           <- M1_tmp
    colnames(M1[[spnames[s]]]) <- paste("Age",1:nages[s])
    
    M1[[spnames[s]]] <- M1[[spnames[s]]]%>%
      mutate(model = mode,type="M1",species = spnames[s], year = NA)%>%
       relocate(model,species,type, year)
  }
  ...
  for(s in 1:3){
    eval(parse(text = paste0("M2_tmp <- ceattle_2$M2_",s," %>%data.frame()")))
    eval(parse(text = paste0("M1_tmp <- ceattle_2$M1_",s," %>%data.frame()")))
    M_total[[spnames[s]]]  <- M2_tmp*0
    for(i in 1:nrow(M2_tmp)){
      M_total[[spnames[s]]][i,] <- (M2_tmp[i,])+(M1_tmp)
    }
    colnames(M_total[[spnames[s]]]) <- paste("Age",1:nages[s])
     M1[[spnames[s]]] <- M1[[spnames[s]]]%>%
       mutate(model = mode,type="Total M",species = spnames[s], year = NA)%>%
       relocate(model,species,type, year)
  }
  if(mode == "Multispecies"){
    
    write.csv(M1, file = file.path("Figs/M1_multispp.csv"))
    write.csv(M2, file = file.path("Figs/M2_multispp.csv"))
    write.csv(M_total, file = file.path("Figs/M_total_multispp.csv"))
    
    save(M1, file = file.path("Figs/M1_multispp.Rdata"))
    save(M2, file = file.path("Figs/M2_multispp.Rdata"))
    save(M_total, file = file.path("Figs/M_total_multispp.Rdata"))
    
  }
  if(mode == "Singlespecies"){
    
    write.csv(M1, file = file.path("Figs/M1_singlespp.csv"))
    write.csv(M2, file = file.path("Figs/M2_singlespp.csv"))
    write.csv(M_total, file = file.path("Figs/M_total_singlespp.csv"))
    
    save(M1, file = file.path("Figs/M1_singlespp.Rdata"))
    save(M2, file = file.path("Figs/M2_singlespp.Rdata"))
    save(M_total, file = file.path("Figs/M_total_singlespp.Rdata"))
...
<!-- # `r thisYr` Climate-enhanced multispecies Stock Assessment for walleye pollock, Pacific cod, and arrowtooth flounder in the Gulf of Alaska -->
...
*November `r thisYr`* | [grant.adams\@noaa.gov](mailto:grant.adams@noaa.gov){.email}
Alaska Fisheries Science Center, National Marine Fisheries Service, NOAA, 7600 Sand Point Way N.E., Seattle, Washington 98115  


**Suggested citation:**  _Adams, G., K. K. Holsman, K. Shotwell (`r thisYr`) Climate-enhanced multispecies stock assessment for walleye pollock, Pacific cod, and arrowtooth flounder in the Gulf of Alaska. North Pacific Fishery Management Council, Anchorage, AK._
...
![](`r "Figs/Table2023vals.jpg"`){width=100%}

\pagebreak

# Overview

This is a three species stock assessment for walleye pollock (*Gadus chalcogrammus*), Pacific cod (*Gadus macrocephalus*) and arrowtooth flounder (*Atheresthes stomias*), from the Gulf of Alaska, Alaska updated from Adams et al.(2022). Results are presented from models estimated and projected without trophic interactions (single-species mode, SSM) and with trophic interactions (multispecies mode, MSM). The main features and settings for this multispecies model include:

-   Predation natural mortality is age specific and annually varying (M2). Estimated residual (non-predation) natural mortality (M1) is age specific but not-annually varying and differs slightly from current assessments for each species (see Table 3 below).
-   Predator overlap index is set to 1 for all species (i.e., all prey are available to all predators).
-   Data and model parameterization are based on the `r thisYr` SAFE report.
...
-   Survey biomass and harvest from the GOA were updated through `r thisYr`.
...
\begin{equation} 
~~~~~~d_{i,y}=e^{(\alpha_{d,i,y}+\alpha0_{d,i}+\beta_{d,i}T_y) }
(\#eq:vonbd)
\end{equation} 

where $\alpha0_{d,i}$ and $\alpha_{d,i,y}$ represent the mean $d$ intercept and $\beta_{d,i}$ is the coefficient for the residual effect of temperature on the $d$ consumption parameter (see [https://github.com/NOAA-REEM/vonBEE](https://github.com/NOAA-REEM/vonBEE) for code and examples (under development until after publication of the revised analysis in 2024). We chose this formulation based on the empirical relationship between temperature and consumption, assuming that $d$ would capture the differential effects of temperature on growth, and that waste rates scale proportionally with weight but do not vary over time with diet or temperature (i.e. $K$ is constant but $d$ can vary with temperature). This formulation allows both the slope and asymptotic limit of growth to vary with temperature (Fig. \@ref(fig:egvonb)). Similar approaches, with slightly different modifications to the VBGF, including temperature and prey specific terms for $d$ and $K$, respectively, have been used elsewhere to evaluate climate impacts on fish growth (e.g., Cheung et al., 2015; Hamre, 2003).



```{r egvonb, echo=FALSE, out.width="60%", fig.align = "center", fig.cap="Example fit of the VonB with Temperature model (here for EBS pollock where bottom temperature (Temp) and Temp2 are predictor variables."}

knitr::include_graphics("Figs/vonB_plk_model_Temp.jpg")
```
...
\begin{equation} 
~~~~\mathrm{ABC}_{x,i,y}=\sum_j^{A_i}((F_{\mathrm{ABC},x,i}^*s_{ij}^\mathrm{f}/Z_{x,ij,y})(1-e^{-Z_{x,ij,y}})N_{x,ij,y} W_{ij,y})
(\#eq:abc)
\end{equation} 
...
The multispecies mode of the model achieved a slightly higher overall fit to the data (i.e., lower negative log-likelihood with the same number of estimated parameters for both models) for pollock and similar fits to the data for Pacific cod and arrowtooth. Both models fit annual total catch for all three species closely (`r round(mean(as.numeric(cor_table_0[3,])),3)`; Fig. \@ref(fig:Catch)). We observed similar fits to survey biomass and age composition data from the single-species (i.e.,$\mathrm{M2}_{ij,y}$ set to 0, hereafter single-species model) and multispecies modes of CEATTLE (Figs. \@ref(fig:Biomass), \@ref(fig:agecomp1)-\@ref(fig:fishagecomp3)). Although both models predicted similar historical total and female spawning biomass, inclusion of trophic interactions in the multispecies model resulted in slightly higher estimates of total biomass for pollock (Fig. \@ref(fig:Biomass)).
...
| Fishery age composition|$-\sum_i{n_i}\sum_y{\sum_j{(O^f_{ij,y}+v)\ln(\hat{O}^f_{ij,y}+v)}}$|Fishery observer data (1979–`r thisYr`), except `r paste(setdiff(years,as.numeric(ceattle_0$yrs_fsh_comp_1)),collapse=", ")` for pollock and  `r paste(setdiff(years,as.numeric(ceattle_0$yrs_fsh_comp_3)),collapse=", ")
` for arrowtooth|
| **Penalties**   |||
...
Diet analyses reveal that there are temporal shifts in the diets of juvenile and adult groundfish associated with marine heatwaves (MHW) in the region (see `r thisYr` Ecosystem Status Report for more information). Changes in diet composition combined with temporal trends in ration (Figs. \@ref(fig:ration), \@ref(fig:diet)) indicates that Pacific cod consumption of pollock, Pacific cod conspecifics, and crab increased during the recent MHW (Fig. \@ref(fig:preyeaten)). Similarly, pollock consumption of zooplankton has also increased over the past decade (Fig. \@ref(fig:preyeaten))
...
```{r rscurve0, echo=FALSE, out.width="75%", fig.align = "center", fig.cap="Spawningstock biomass and recruitment for the single-species model. Color scale text indicates cohort years summer bottom temperature (from the Bering10K ROMSNPZ model) for the EBS in deg. C. Lines represent climate-naive Ricker stock recruit curves"}
...
We found some support for climate driven variation in recruitment with slightly higher support for Pacific cod as evidenced from a smaller set of top AICc selected models for that species (`r as.numeric(aic_n[2,2:3])[1]` and `r as.numeric(aic_n[2,2:3])[2]`, for single- and multi-species modes, respectively), while pollock had `r as.numeric(aic_n[1,2:3])[1]` and `r as.numeric(aic_n[1,2:3])[2]` top set of models (for single- and multi-species modes, respectively) and Arrowtooth single- and multi-species modes had `r as.numeric(aic_n[3,2:3])[1]` and `r as.numeric(aic_n[3,2:3])[2]`, respectively (Tables \@ref(tab:topRicker),\@ref(tab:topmodels)).


```{r topRicker, eval=T, echo=FALSE, include =T, layout="l-body-outset"}
# library(knitr)
# library(kableExtra)
...
table.tbl0 <- AIC_0_sub1%>%select(-aicc, -mode,-set,-rank)%>%relocate(Species)%>%arrange(Species)|> distinct()
table.tbl2 <- AIC_2_sub1%>%select(-aicc, -mode,-set,-rank)%>%relocate(Species)%>%arrange(Species)|> distinct()
subhead1 <- subhead2 <-table.tbl0[1,]
for(c in 2:ncol(table.tbl0)) subhead1[,c]<-""
 subhead1[,1]<-"a) Single-species"
for(c in 2:ncol(table.tbl0)) subhead2[,c]<-""
 subhead2[,1]<-"b) Multi-species"
 
table.tbl <- rbind(subhead1,table.tbl0,subhead2,table.tbl2)%>%dplyr::rename(Covariates = model2)


table.tbl%>%
  kbl( row.names = FALSE, padding = 2, format = "latex",booktabs = T,
      align = "lllcccllc", 
      caption="Top selected Ricker recruitment models for each species and mode.", 
      table.attr = "style='width:30%;'") %>% 
  kableExtra::kable_styling(latex_options="scale_down","striped") %>%
  row_spec(0,bold=TRUE, extra_latex_after = "\\arrayrulecolor{black}") %>% 
  row_spec(1,bold=TRUE, extra_latex_after = "\\arrayrulecolor{gray}") %>% 
  row_spec(2, extra_latex_after = "\\arrayrulecolor{white}") %>% 
  row_spec(( table.tbl0 %>% nrow()+1 ), extra_latex_after = "\\arrayrulecolor{black}") %>% 
  row_spec(( table.tbl0 %>% nrow()+2 ),bold=TRUE, extra_latex_after = "\\arrayrulecolor{gray}") %>% 
   row_spec(( table.tbl0 %>% nrow()+3 ), extra_latex_after = "\\arrayrulecolor{white}") %>% 
  row_spec(( table.tbl %>% nrow() ) , extra_latex_after = "\\arrayrulecolor{black}")

```


```{r topmodels, eval=T, echo=FALSE, include =T, layout="l-body-outset"}

table.tbl0 <- AIC_0_sub2%>%select(-aicc, -mode,-set,-rank)%>%relocate(Species)%>%arrange(Species)|> distinct()
table.tbl2 <- AIC_2_sub2%>%select(-aicc, -mode,-set,-rank)%>%relocate(Species)%>%arrange(Species)|> distinct()
subhead1 <- subhead2 <-table.tbl0[1,]
for(c in 2:ncol(table.tbl0)) subhead1[,c]<-""
 subhead1[,1]<-"a) Single-species"
for(c in 2:ncol(table.tbl0)) subhead2[,c]<-""
...
table.tbl%>%
  kbl( row.names = FALSE, padding = 2, format = "latex",booktabs = T,
      align = "lllcccllc", caption="Top selected (all) recruitment models for each species and mode." , 
      table.attr = "style='width:30%;'") %>% 
  #add_header_above(c("a) Single-species"=2, " ", " ")) %>%
  kableExtra::kable_styling(latex_options="scale_down","striped") %>%
  row_spec(0,bold=TRUE, extra_latex_after = "\\arrayrulecolor{black}") %>% 
  row_spec(1,bold=TRUE, extra_latex_after = "\\arrayrulecolor{gray}") %>% 
  row_spec(2, extra_latex_after = "\\arrayrulecolor{white}") %>% 
...
Common predictor variables in the top selected recruitment models included summer and fall sea surface temperature ('(Su)SST' and '(Fa)SST'), summer bottom temperature ('(Su)BT'), fall Neocalanus ('(Fa)NCaS'), winter and spring Euphausiids ('(Wi)EupS, (Sp)EupS'), winter pH ('(Wi)pH ') as well as spring and fall large Zooplankton ('(Sp)largeZoop' and '(Fa)largeZoop', respectively; Figs. \@ref(fig:rects0), \@ref(fig:rects2)). This is consistent with previous studies that used a slightly different NPZ formulation (Holsman et al. 2020, Kearney et al. 2020), as well as field studies that have postulated the importance of lower trophic productivity for groundfish larval and juvenile survival in the Bering Sea (Duffy-Anderson et al. 2017). In general, recruitment was found to be inversely correlated with temperature and positively correlated with indices of ecosystem productivity (copepod and euphausiid abundance; Fig. \@ref(fig:vars)).



```{r rects0, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Recruitment covariate fits for the multispecies model updated for this year. Recruiment estimated from this year's Bering Sea CEATTLE model in single-species mode (dashed lines, top row) for each species and fits based on a climate-naive Ricker RS model, top AIC selected climate-driven model for each species, and climate-driven model with the highest R2 fit. In the bottom panel the combined covariates for the top R2 and AIC models are plotted for comparison."}

knitr::include_graphics("Figs/rec_models_0.jpg")
```


```{r rects2, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Recruitment covariate fits for the multispecies model updated for this year. Recruiment estimated from this year's Bering Sea CEATTLE model in multi-species mode (dashed lines, top row) for each species and fits based on a climate-naive Ricker RS model, top AIC selected climate-driven model for each species, and climate-driven model with the highest R2 fit. In the bottom panel the combined covariates for the top R2 and AIC models are plotted for comparison."}

knitr::include_graphics("Figs/rec_models_2.jpg")
...
The single- and multispecies models estimate similar fishing mortality rates for pollock that have remained relatively stable at around `r round(apply(mnF1,2,mean)[2],2)` since the early 1980s (Fig. \@ref(fig:plotF)). Both models also estimate low and relatively steady fishing mortality rates for arrowtooth flounder (i.e., \~ `r round(apply(mnF3,2,mean)[2],2)`). Fishing mortality for Pacific cod is generally higher than pollock or arrowtooth, and varies over time (`r round(apply(ceattle_2$F_avg,1,mean)-apply(ceattle_2$F_avg,1,sd),2)[2]`-`r round(apply(ceattle_2$F_avg,1,mean)+apply(ceattle_2$F_avg,1,sd),2)[2]`). Fishing mortality for Pacific cod declined substantially between 2012-2021 with indication of slight increases in 2022-`r thisYr`  (Fig. \@ref(fig:plotF)).

```{r plotF, echo=FALSE, out.width="70%", fig.align = "center", fig.cap="Timeseries of single- and multispecies (solid and dashed lines, respectively) CEATTLE model estimates of fishing mortality rate for eastern Being Sea walleye pollock, Pacific cod, and arrowtooth flounder. Note that the single- and multispecies lines for pollock and arrowtooth flounder overlap."}
...
```{r deltassb, echo=FALSE, out.width="90%", fig.align = "center", fig.cap="Projections of spawning stock biomass relative to this year's SSB."}

knitr::include_graphics("Figs/delta_tsthiYr.png")
```
...
That was Figure  \@ref(fig:Catch).
That was Figure  \@ref(fig:Biomass).
That was Figure  \@ref(fig:Selectivity).



```{r agecomp1, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Survey age compostitions for walleye pollock. Bars represent observed values, black and gray points represent single- and multispecies fits to the data, respectively."}

knitr::include_graphics("Figs/R_figures/surv_age_comp_by_yr_1v2.jpg")
```

```{r agecomp2, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Survey length compostitions for Pacific cod Bars represent observed values, black and gray points represent single- and multispecies fits to the data, respectively."}

knitr::include_graphics("Figs/R_figures/surv_age_comp_by_yr_2v2.jpg")
```


```{r agecomp3, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Survey length compostitions for arrowtooth flounder Bars represent observed values, black and gray points represent single- and multispecies fits to the data, respectively."}

knitr::include_graphics("Figs/R_figures/surv_age_comp_by_yr_3v2.jpg")
```


```{r fishagecomp1, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Fishery age compostitions for walleye pollock. Bars represent observed values, black and gray points represent single- and multispecies fits to the data, respectively."}

knitr::include_graphics("Figs/R_figures/fsh_age_comp_by_yr_1v2.jpg")
```


```{r fishagecomp2, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Fishery age compostitions for Pacific cod. Bars represent observed values, black and gray points represent single- and multispecies fits to the data, respectively."}

knitr::include_graphics("Figs/R_figures/fsh_age_comp_by_yr_2v2.jpg")
```



```{r fishagecomp3, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Fishery age compostitions for arrowtooth flounder. Bars represent observed values, black and gray points represent single- and multispecies fits to the data, respectively."}

knitr::include_graphics("Figs/R_figures/fsh_age_comp_by_yr_3v2.jpg")
```


```{r ssbplot0, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="Single-species CEATTLE model projections of unfished (B0, dashed lines) and fished spawning stock biomass at the harvest rate corresponding to the ABC proxy (B40, solid lines) for the climate naive scenario (mnhind), high carbon mitigation (ssp126) and low carbon mitigation (ssp585) scenarios. B0 from the climate naive model (left panel) was used to determine the target biomass B40 each species (rows). The lines in the two carbon mitigation scenarios represent different (earth system model) realizations which differentially impact recruitment and weight at age in the model. Harvest is set at Ftarget for each species.Projections assume no adaptation by the species, fishery, or fishery management. See Holsman et al. 2020 for more information and climate informed projections."}

knitr::include_graphics("Figs/ssb_plot0.png")
```

```{r ssbplot2, echo=FALSE, out.width="100%", fig.align = "center", fig.cap= "Multi-species CEATTLE model projections of unfished (B0, dashed lines) and fished spawning stock biomass at the harvest rate corresponding to the ABC proxy (B40, solid lines) for the climate naive scenario (mnhind), high carbon mitigation (ssp126) and low carbon mitigation (ssp585) scenarios. B0 from the climate naive model (left panel) was used to determine the target biomass B40 each species (rows). The lines in the two carbon mitigation scenarios represent different (earth system model) realizations which differentially impact recruitment and weight at age in the model. Harvest is set at Ftarget for each species.Projections assume no adaptation by the species, fishery, or fishery management. See Holsman et al. 2020 for more information and climate informed projections."}

knitr::include_graphics("Figs/ssb_plot2.png")
```

```{r ssbplotzoom0, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="As in previous figures but with closer detail at this year. Spawning stock biomass for each species at the start of each model year (single-species). Vertical line represents end of this year and the start of next year"}

knitr::include_graphics("Figs/ssb_plot_zoom0.png")
```

```{r ssbplotzoom2, echo=FALSE, out.width="100%", fig.align = "center", fig.cap="As in previous figures but with closer detail at this year. Spawning stock biomass for each species at the start of each model year (multi-species). Vertical line represents end of this year and the start of next year"}

knitr::include_graphics("Figs/ssb_plot_zoom2.png")
```


\pagebreak

```{r vonB, echo=FALSE, out.width="65%", fig.align = "center", fig.cap="Temperature-dependent Von Bertalanffy parameter (parm) estimates, standard deviation in parameter estimates (stdev), and confidence intervals (CI) from Holsman and Aydin, 2015."}

knitr::include_graphics("Figs/vonBtable.jpg")
```
