...
dir0 <- "C://Users//Grant Adams//Documents//GitHub//RceattleRuns//GOA//Data//Pcod 2020//GOAPcod2020SEP6.1_10P" # 3.24 Model
...
wt_all <- rbind(wt_flt_4, wt_flt_5, wt_flt_1, wt_flt_2, wt_flt_3, wt_flt_b, wt_flt_ssb)
wt_all = wt_all[,c("Wt_name","Wt_index","Species","Yr","Sex",1:10)]
wt_all <- wt_all[which(wt_all$Yr >= 1977 & wt_all$Yr <= 2020),]
write.csv(wt_all, file = "Data/Pcod 2020/GOAPcod2020SEP6.1_10P_wtatage.csv")

# Time series of biomass
write.csv(mod0$timeseries, file = "Data/Pcod 2020/GOAPcod2020SEP6.1_10P_time_series.csv")
...
# Group age-0 and age-1
alkbegin[2,] <- alkbegin[2,] + alkbegin[1,]
alkbegin <- alkbegin[-1,]

alkmid[2,] <- alkmid[2,] + alkmid[1,]
alkmid <- alkmid[-1,]
...
writexl::write_xlsx(alk, path = "Data//Pcod 2020//GOAPcod2020SEP6_1_10P_alk.xlsx")



# N-at-age
natage <- mod0$natage
natage <- natage[which(natage$Yr >= 1977 & natage$Yr <= 2020),]

natage_list <- list(begininng = natage[which(natage$`Beg/Mid` == "B"),], mid = natage[which(natage$`Beg/Mid` == "M"),])
writexl::write_xlsx(natage_list, path = "Data/Pcod 2020/GOAPcod2020SEP6.1_10P_natage.xlsx")
