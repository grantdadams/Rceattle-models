...
!!CLASS ofstream evalout("atf_gen2.mcmc.out");       //wasgen
 ...
 init_number offset_const //(66) a constant to offset zero values
  ...
  int styr_rec;
  3darray obs_p_fish_norm(1,2,1,nobs_fish,1,nlen)
  init_int eof_marker
...
//fishery offset
  ...
  for(k=1; k<=2;k++)
  {
    offset(1) -= (nsamples_fish(1,i) + nsamples_fish(2,i)) *obs_p_fish_norm(k,i) * log(obs_p_fish_norm(k,i)+offset_const); //this multiplies elements together then sums them up.
  }
  }
  }
    ...
    for(k=1;k<=2;k++)
    {
      for (i=1; i <= nobs_fish; i++)
      {
        ...
        //fishery length likelihood fitting
          length_like(1) -= (nsamples_fish(1,i) + nsamples_fish(2,i))*(offset_const+obs_p_fish_norm(k,i))*log(pred_p_fish(k,ii)+offset_const);
      }
    }
