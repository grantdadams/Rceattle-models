...
void model_parameters::preliminary_calculations(void)
{
 ...
 // magnitude depends on nsamples(sample size) and p's_
  ...
  for(k=1; k<=2;k++)
  {
    offset(1) -= (nsamples_fish(1,i) + nsamples_fish(2,i))*obs_p_fish(k,i) * log(obs_p_fish(k,i)+offset_const); //this multiplies elements together then sums them up.
  }
  }
  }
  if(assess==3)
  ...
  for(i=1;i<=nsurv;i++){
	for(j=1;j<=nobs_srv_length(i);j++){
        ...
        offset(i+1)-= (nsamples_srv_length_fem(i,j) + nsamples_srv_length_mal(i,j)) *obs_p_srv_length_fem(i,j)*log(obs_p_srv_length_fem(i,j)+offset_const)
                   +(nsamples_srv_length_fem(i,j) + nsamples_srv_length_mal(i,j))*obs_p_srv_length_mal(i,j)*log(obs_p_srv_length_mal(i,j)+offset_const);
	}
  }
  //survey age offsets
  for (i=1;i<=nsurv_aged;i++)
  {
	...
	offset(i+nsurv+1)-=(nsamples_srv_age(i,1,j) + nsamples_srv_age(i,2,j))*obs_p_srv_age_fem(i,j)*log(obs_p_srv_age_fem(i,j)+offset_const)+
	             (nsamples_srv_age(i,1,j) + nsamples_srv_age(i,2,j))*obs_p_srv_age_mal(i,j)*log(obs_p_srv_age_mal(i,j)+offset_const);
    }
  }
}
...
void model_parameters::evaluate_the_objective_function(void)
{
    ...
    for(k=1;k<=2;k++)
    {
      for (i=1; i <= nobs_fish; i++)
      {
        ...
        //fishery length likelihood fitting
          length_like(1) -= (nsamples_fish(1,i) + nsamples_fish(2,i))*(offset_const+obs_p_fish(k,i))*log(pred_p_fish(k,ii)+offset_const); // K < 3
      }
    }
  ...
  //survey length composition fitting
   for (i=1;i<=nsurv;i++)
   {
     for (j=1;j<=nobs_srv_length(i);j++)
     {
	   length_like(i+1)-=(((nsamples_srv_length_fem(i,j) + nsamples_srv_length_mal(i,j))*(offset_const+obs_p_srv_length_fem(i,j))*log(pred_p_srv_len_fem(i,j)+offset_const))
	                      +((nsamples_srv_length_fem(i,j) + nsamples_srv_length_mal(i,j))*(offset_const+obs_p_srv_length_mal(i,j))*log(pred_p_srv_len_mal(i,j)+offset_const)));
	 }
  ...
  for (i=1;i<=nsurv_aged;i++)
  {
	for (j=1;j<=nobs_srv_age(i);j++)
	{
   age_like(i)-=(nsamples_srv_age(i,1,j) + nsamples_srv_age(i,2,j))*(offset_const+obs_p_srv_age_fem(i,j))*log(pred_p_srv_age_fem(i,j)+offset_const)+
                  (nsamples_srv_age(i,1,j) + nsamples_srv_age(i,2,j))*(offset_const+obs_p_srv_age_mal(i,j))*log(pred_p_srv_age_mal(i,j)+offset_const);
	}
