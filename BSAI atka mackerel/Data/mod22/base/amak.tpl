...
//---------------------------------------------------------------------------
 ...
 // Fishing mortality parameters
  ...
  init_bounded_matrix fmort(1,nfsh,styr,endyr,0.00,2.,phase_fmort)
  ...
  for (k=1;k<=nfsh;k++)
  {
    ...
    if (active(log_selcoffs_fsh(k)))
    {
      for (i=1;i<=n_sel_ch_fsh(k);i++)
      {
        ...
        sel_like_fsh(k,1) += curv_pen_fsh(k)*norm2(first_difference( first_difference(log_sel_fsh(k,iyr)))); //log_sel_fsh is array(fishery, years, ages)
        ...
        for (j=seldecage;j<=nagestmp;j++)
        {
          ...
          if (difftmp > 0.)
            sel_like_fsh(k,3)    += .5*square( difftmp ) / seldec_pen_fsh(k); // Dome penalty
        }
...
// Function to write out data file for projection model....
 ...
 seltmp.initialize();
 Fratio.initialize();
 for (i = endyr-5;i<endyr;i++)
   for (k=1;k<=nfsh;k++)
   {
     Fratio(k) += (mean(F(k,i))) ;
     sumF      += Fratio(k) ;
     seltmp(k) += value(sel_fsh(k,i))/max(value(sel_fsh(k,i)));
   }
 sumF /= 5.;
 seltmp /= 5.;
