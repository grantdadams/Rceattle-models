  ...
  // Write time series data (1963-2002)
      ...
      write_s_yrcol(sfile, value(M_year), "M.Year", styr, endyr, 0);
  ...
  // ------------------ START FULL M MATRIX -------------------------
  // Matrix of natural mortality, with ages as columns & years as rows
  write_s_matrix(sfile, value(M), "M", styr, endyr, 1, nages);
  ...
  // ------------------ START N-AT-LENGTH MATRIX -------------------------
  // Matrix of numbers at length, with size bins as columns & years as rows
  write_s_matrix(sfile, value(num_len_f), "numlen.f", styr, endyr, 1, nlenbins);
  write_s_rownames_vec(sfile, len_bin_labels, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
  write_s_matrix(sfile, value(num_len_m), "numlen.m", styr, endyr, 1, nlenbins);
  write_s_rownames_vec(sfile, len_bin_labels, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
...
//      write_s_rownames_int(sfile, 1, 20, "df", 0);
 ...
 // write start of age based selectivity curves
    ...
    write_s_yrcol(sfile, value(fish5_sel_f), "fish5sel.f", 1, nages, 0);
    write_s_yrcol(sfile, value(fish5_sel_m), "fish5sel.m", 1, nages, 0);
    write_s_yrcol(sfile, value(srv1_sel_f), "srv1sel.f", 1, nages, 0);
    write_s_yrcol(sfile, value(srv1_sel_m), "srv1sel.m", 1, nages, 0);
    write_s_yrcol(sfile, value(srv10_sel_f), "srv10sel.f", 1, nages, 0);
    write_s_yrcol(sfile, value(srv10_sel_m), "srv10sel.m", 1, nages, 0);
  ...
  // ------------------ age_length matrices -------------------------
  // Matrix of numbers at age, with ages as columns & years as rows
  write_s_matrix(sfile, value(sizeage1_f), "sizeage.f.block1", 1, nlenbins, 1, nages);
  write_s_rownames_vec(sfile, aa, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
  write_s_matrix(sfile, value(sizeage1_m), "sizeage.m.block1", 1, nlenbins, 1, nages);
  write_s_rownames_vec(sfile, aa, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
  write_s_matrix(sfile, value(sizeage2_f), "sizeage.f.block2", 1, nlenbins, 1, nages);
  write_s_rownames_vec(sfile, aa, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
  write_s_matrix(sfile, value(sizeage2_m), "sizeage.m.block2", 1, nlenbins, 1, nages);
  write_s_rownames_vec(sfile, aa, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
  write_s_matrix(sfile, value(sizeage3_f), "sizeage.f.block3", 1, nlenbins, 1, nages);
  write_s_rownames_vec(sfile, aa, 1, nlenbins, "ma", 0);  // this writes  COLUMN names
  write_s_matrix(sfile, value(sizeage3_m), "sizeage.m.block3", 1, nlenbins, 1, nages);
  write_s_rownames_vec(sfile, aa, 1, nlenbins, "ma", 0);  // this writes  COLUMN names 
// ------------------------------growth and maturity curves ----------------------------------------
 // write start of age based selectivity curves
    sfile << "growthmat = structure(list(" << endl;
    write_s_yrcol(sfile, wt_f1, "wt.f.block1", 1, nages, 0);
    write_s_yrcol(sfile, wt_m1, "wt.m.block1", 1, nages, 0);
    write_s_yrcol(sfile, wt_f2, "wt.f.block2", 1, nages, 0);
    write_s_yrcol(sfile, wt_m2, "wt.m.block2", 1, nages, 0);
    write_s_yrcol(sfile, p_mature1, "mage.block1", 1, nages, 0);
    write_s_yrcol(sfile, p_mature2, "mage.block2", 1, nages, 0);
    write_s_yrcol(sfile, p_mature3, "mage.block3", 1, nages, 1);
    write_s_rownames_vec(sfile, aa, 1, nages, "df", 0);
...
//	write_s_rownames_int(sfile, styr, endyr, "df", 0);
  ...
  // Write start of likelihood components vector:
      ...
      sfile << age_like(1) << cc << age_like(2) << cc << age_like(3) << cc << age_like(4) << cc << age_like(5) << cc << age_like(6) << cc << age_like(7) << cc << age_like(8) << cc << age_like(9) << cc << age_like(10) << cc << age_like(11) << cc << age_like(12) << cc << age_like(13) << cc << age_like(14) << cc << surv_like(1) << cc<<surv_like(2) << cc<<surv_like(3) << cc<<surv_like(4) << cc<<surv_like(5) << cc<<surv_like(6) << cc<<surv_like(7) << cc<<surv_like(8) <<cc<<obj_fun<<cc<<ssqcatch<<cc<<rec_like<<cc<<F_mort_regularity<<cc<<priors(4);
        sfile << ")," << endl; 
        sfile << ".Names = c('L.fish1age','L.surv1age','L.fish1sizef','L.fish1sizem','L.fish2size','L.fish3sizef','L.fish3sizem','L.fish4size','L.surv1sizef','L.surv1sizem','L.surv2sizef','L.surv2sizem','L.surv7sizef','L.surv7sizem','L.surv1','L.surv2','L.surv3','L.surv4','L.surv5','L.surv6','L.surv7','L.surv8','obj.fun','Catch','Rec.Pen','F.reg','M.prior'";
  ...
  //Write likelihood component values:
   ...
   // write some parameter values
        ...
        sfile << "phases = structure(c(";
        sfile << ph_mean_rec << cc << ph_recdev << cc<< ph_Rzero<< cc<< ph_steepness<< cc<< ph_sigr << cc<< ph_avg_F << cc<< ph_Fdev << cc<< ph_F50 << cc << ph_m<< cc << ph_Mdevs << cc<<ph_Mdevs_age<<cc<<ph_mdelta<< cc<< ph_fish_sel << cc<< ph_fish2_sel << cc<< ph_ifq << cc<< ph_fish_sel_delt<< cc<< ph_fish_sel_delt_alt<< cc<< ph_srv1_sel << cc<< ph_srv2_sel<< cc<< ph_LL_block2 << cc << ph_srv_sel_delt<< cc<< ph_srv_sel_delt_alt << cc<< ph_q_srv1<< cc << ph_q_srv2 << cc<<ph_q_srv3<<cc<<ph_q_srv4<<cc<<ph_q_srv5 << cc<< ph_q_srv6 << cc<< ph_q_srv7<< cc << ph_q_srv8<< cc << ph_srv2_q2;
        sfile << ")," << endl; 
        sfile << ".Names = c('ph.mean.rec','ph.recdev','ph.Rzero','ph.steepness','ph.sigr','ph.avg.F','ph.Fdev','ph.F50','ph.m','ph.Mdevs','ph.Mdevs.age','ph.mdelta','ph.fish.sel','ph.fish2.sel','ph.ifq','ph.fish.sel.delt','ph.fish.sel.delt.alt','ph.srv1.sel','ph.srv2.sel','ph.LL.block2','ph.srv.sel.delt','ph.srv.sel.delt.alt', 'ph.q.srv1','ph.q.srv2','ph.q.srv3','ph.q.srv4','ph.q.srv5','ph.q.srv6','ph.q.srv7','ph.q.srv8','ph.srv2.q2'";
         sfile << "))" << endl;   // Close vector of parameter values:
