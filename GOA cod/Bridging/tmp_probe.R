...
cod_pcod_fixed <- Rceattle::fit_mod(
  ...
  # TRY: with growthFun_est_spec + log()-wrapped bounds (K/L1/sd_*) and
  # natural-scale bounds (Linf), build_bounds succeeds but FP TOTAL NLL gap
  # worsens to +1301 (vs +719 with growthFun_spec). Cause likely:
  # (a) linkage priors on K + Linf add ~5 NLL (visible as Parm priors -3.9)
  # (b) sel/N propagation reads log_growth_pars through linkage path
  #     and small numerical differences accumulate -> +574 NLL on Catch.
  # Reverting to growthFun_spec while we trace. To re-attempt with est_spec
  # unified, switch this back to growthFun_est_spec and `inits$beta_linkage`
  # is already grafted from mod0_est.
