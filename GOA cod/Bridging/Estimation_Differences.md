...
| 20 | Time-varying sel/q "IID" overpenalizes vs SS3 "BlockDev" replacement | Parm_devs SS3 6 vs Rce 497 (free IID); BlockDev closes to ~94; param count 1672 → **291** (within 39 of SS3's 330) | ✅ | SS3 fires N(0, σ) prior ONCE per block-replacement parameter (4–5 per fleet); Rce's IID prior fires once per (fleet, slot, year) — 1590+ priors. Resolved as a proper Rceattle enum (`Time_varying_sel = "BlockDev"`, value 6 in `tv_sel_map`; same for `Time_varying_q = "BlockDev"` value 5). **Cpp**: per-cell prior_weight tensors `DATA_ARRAY(sel_inf_dev_prior_weight)`, `DATA_ARRAY(log_sel_slp_dev_prior_weight)`, `DATA_MATRIX(index_q_dev_prior_weight)` in [ceattle_v01_11.cpp:286-296](../../Rceattle/src/TMB/ceattle_v01_11.cpp#L286); IID prior contributions multiplied by per-cell weight. Defaults = 1.0 preserve existing IID behavior bit-identically. **R**: `build_map_selectivity` DoubleNormal branch + `build_map_catchability` q branch handle `"BlockDev"` mode — NA-lock every dev cell first, then factor-share one estimable label per sub-block. `rearrange_data` auto-populates `*_dev_prior_weight = 1/N` from `Selectivity_block` for fleets in BlockDev mode. `build_map_f_and_data_weights` NA-locks sel/q dev cells for `Fleet_type == "Off"` fleets so they don't leak through the default sequential map. **Driver**: `populate_selectivity_block()` writes per-(fleet, year) sub-block ID from SS3 `Block_Design` into `index_data$Selectivity_block` / `catch_data$Selectivity_block`; both FP and estimate scripts now set `Time_varying_sel = "BlockDev"` for active fleets (FP NLL is bit-identical to the prior IID-with-sentinel path because `sel_dev_log_sd = -999` sentinel skips the prior loop). **Pcod result**: Rce 291 estimable params vs SS3's 330 — gap of 39 is fully explained by missing #10 SS3-three-tier dev-seq layer (63 per-year DEVmult params) minus our 1 fixed `log_Finit` minus a few PHASE-fix slot differences. |
| 21 | SS3 case-1 exponential env-link on q (`env_var&link = 1*100 + var`) ≠ Rce additive log-linear | LLSrv FP: prior workaround +0.34 NLL, EnvExp ✅ machine precision (q ratio 3.5e-6) | ✅ | SS3 `SS_timevaryparm.tpl` case 1 applies `parm_timevary *= exp(beta·env)` to `LnQ_base`, giving `q[y] = exp(LnQ_base · exp(beta·env[y]))` — nested-exponential, NOT representable as Rce's earlier `Environmental` mode (`log(q) = LnQ + β·env`, additive log-linear). Resolved by new `EnvExp` Catchability mode (= 7 in [R/0-constants.R](../../Rceattle/R/0-constants.R)): cpp branch at [ceattle_v01_11.cpp:583-596](../../Rceattle/src/TMB/ceattle_v01_11.cpp#L583) computes the nested form; build_map_catchability turns on the env-column slope same way as mode 5; data_check + clean_data accept the overloaded `Time_varying_q = "<env_col_idx>"` field. **Driver**: test-script `init_from_ss3` reads `LnQ_base_<fleet>(<num>)_ENV_add` from `parlist$Q_parms` (SS_readpar_3.30 places tv-q rows here) and writes into `inits$index_q_beta[fleet, env_<v>]`; SS3 `env_var&link` decoded as `var = evl %% 100`. **Pcod FP impact**: Survey +0.34 → −0.30, Length comp +35 → +0.76, Catch +330 → +238, Bio rel err 3.0e-2 → 9.8e-4, TOTAL +539 → +370 (−169 NLL). Per-year q[LLSrv] vs SS3 Calc_Q ratio: 0.999996 .. 1.000003 across all 34 years. |
...
|---|---|---|---|---|---|
| Survey index | -1.79 | -2.09 | -0.30 | 🟡 | #21 EnvExp matches q to machine precision |
| Catch | 1.75 | 239.80 | +238 | 🟠 | EnvExp closed −91; remaining is #4 Pope's vs Baranov |
| Length comp | 1336.33 | 1337.09 | **+0.76** | ✅ | EnvExp closed it (was +35 with q_dev workaround) |
| CAAL | 721.20 | 855.73 | +134.5 | 🟠 | #17/#19 closed Srv; fishery residual downstream of #4 |
...
| Parm devs (sel+q) | 6.09 | 0 | -6.09 | 🟡 | sentinel σ skips dev prior in FP path; no spurious q-dev contribution |
| **FP TOTAL** | **2048.07** | **2418.52** | **+370.4** | | post-EnvExp + ageing-error; Bio rel err 9.8e-4 |
...
| BlockDev infrastructure (#20) | Parm devs +491 → +88; param count 1672 → **291** (vs SS3's 330) | Promoted from script-level helper to proper Rceattle enum: `Time_varying_sel = "BlockDev"` (value 6) and `Time_varying_q = "BlockDev"` (value 5) in tv_*_map; `build_map_selectivity` DoubleNormal + `build_map_catchability` handle the factor-sharing; `rearrange_data` auto-populates 1/N prior_weight from `Selectivity_block`. Off-fleet sel/q cells NA-locked in `build_map_f_and_data_weights` to prevent leakage through the default sequential map. |
...
| EnvExp catchability (#21) | Survey +0.34 → −0.30, Length comp +35 → +0.76, TOTAL −169 NLL | New `Catchability = "EnvExp"` mode implementing SS3 case-1 `q = exp(LnQ · exp(β·env))`; replaces the prior per-year q_dev injection workaround for LLSrv |
