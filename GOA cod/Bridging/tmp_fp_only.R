...
sel_dev_sd_num      <- as.numeric(fc$Time_varying_sel_sd)
...
# Srv/LLSrv are NUMBERS-based surveys (Observation_units = 2): the SS3 + Rce
...
# forward-pass sets Time_varying_sel_sd = -1 (sentinel -> skip the
...
cod_pcod$fleet_control$Time_varying_sel_sd[active_fi] <- 1.0
cat(sprintf("\n[estimation] Reset Time_varying_sel_sd to 1.0 for %d active fleets (sentinel off)\n",
            length(active_fi)))
