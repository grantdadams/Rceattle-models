...
# The workbook ships endyr = 2019 but carries catch, index and composition data
# through 2023. The likelihood scores only Year <= endyr, so those four years
# were read and never fit, and the MSE treated them as projection years it had
# already been handed catch for. Conditioning through 2023 is what this
# assessment is for. NByageFixed and ration_data run to 2066, weight to 2024 and
# emp_sel to 2023, so every fixed input covers the longer hindcast.
CSL_SBF_ATF_hakedata_DM$projyr <- 2025
...
# Hake biomass in the MSE's operating model against the conditioning fit.
plot_biomass(list(mse1_CSL$Sim_1$OM, run_ms_CSL_Mest_prior_DM_CSL_stable),
             model_names = c("MSE OM (Sim 1)", "Conditioned MS model"),
             species = 1)
