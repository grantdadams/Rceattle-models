...
SBF_ATF_hakedata <- Rceattle::read_data(file = "110226_SBF_ATF_Hake.xlsx")
...
run_ms_CSL_Mest$quantities$jnll #2762.643
run_ms_CSL_Mest$quantities$jnll_comp
plot_diet_comp(run_ms_CSL_Mest)
plot_diet_comp2(run_ms_CSL_Mest)

save(run_ms_LN_3, file = "results/models/ATF_SBF/ATF_SBF_ms_LN_estM.Rdata")

run_ms_CSL_Mest_2 <- Rceattle::fit_mod(data_list = test_data,
                                     inits = inits, # Initial parameters from single species ests
                                     map = map,
                                     M1Fun = build_M1(M1_model = 1,
                                                      updateM1 = FALSE,
...
mod_list <- list(ss_run,run_ms_CSL_Mest, run_ms_CSL_Mest_2)
mod_names <- c("ss_run", "run_ms_LN_estM", "run_ms_CSL_Mest_2")
...
plot_biomass(Rceattle = mod_list, model_names = mod_names, add_ci = TRUE, species =1, file = "Manuscript_1/figures/B_plot")
plot_recruitment(Rceattle = mod_list, model_names = mod_names, add_ci = TRUE, species =1, file = "Manuscript_1/figures/Rec_plot") #this looks pretty different
plot_ssb(Rceattle = mod_list, model_names = mod_names, add_ci = TRUE, species = 1, file = "Manuscript_1/figures/SSB_plot")

plot_diet_comp3(run_ms_CSL_Mest, file = "Manuscript_1/figures/diet_comp_MS")
plot_catch(Rceattle = mod_list, model_names = mod_names, file = "Manuscript_1/figures/diagnostics/catch")
plot_index(Rceattle = mod_list, model_names = mod_names, file = "Manuscript_1/figures/diagnostics/survey_index")
plot_selectivity(Rceattle = mod_list, model_names = mod_names, file = "Manuscript_1/figures/diagnostics/selectivity")
plot_comp(run_ms_CSL_Mest, file = "figures/diagnostics/Comps")
plot_comp(ss_run)
