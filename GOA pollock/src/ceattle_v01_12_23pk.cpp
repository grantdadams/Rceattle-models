...
//  INDEX:
//  0. DSEM
//  1. CEATTLE configuration
//  2. CEATTLE inputs
//  3. CEATTLE parameters
//  4. CEATTLE derived quantities
//  5. CEATTLE initial calculations
//  6. CEATTLE population dynamics
//  7. CEATTLE ration equations
...
Type objective_function<Type>::operator() () {
  ...
  // globals
  Type jnll_dsem = 0;
  Type jnll_gmrf_dsem = 0;
  matrix<Type> loglik_tj_dsem( n_t, n_j );
  ...
  if( options(0)==0 ){
    ...
    // Centered GMRF
    jnll_gmrf_dsem = GMRF(Q_kk)( x_tj - xhat_tj - delta_tj );
    z_tj = x_tj;
    REPORT( Q_kk );
  }else{
    // Rank-deficient (projection) method
    jnll_gmrf_dsem = GMRF(I_kk)( x_tj );
    ...
    for(int j=0; j<n_j; j++){
      ...
      if( familycode_j(j)==1 ){
        ...
        if(!R_IsNA(asDouble(y_tj(t,j)))){
          loglik_tj_dsem(t,j) = dnorm( y_tj(t,j), mu_tj(t,j), sigma_j(j), true );
        }
        SIMULATE{
      ...
      if( familycode_j(j)==2 ){
        ...
        if(!R_IsNA(asDouble(y_tj(t,j)))){
          loglik_tj_dsem(t,j) = dbinom( y_tj(t,j), Type(1.0), mu_tj(t,j), true );
        }
        SIMULATE{
      ...
      if( familycode_j(j)==3 ){
        ...
        if(!R_IsNA(asDouble(y_tj(t,j)))){
          loglik_tj_dsem(t,j) = dpois( y_tj(t,j), mu_tj(t,j), true );
        }
        SIMULATE{
      ...
      if( familycode_j(j)==4 ){
        mu_tj(t,j) = exp(z_tj(t,j));
        if(!R_IsNA(asDouble(y_tj(t,j)))){
          loglik_tj_dsem(t,j) = dgamma( y_tj(t,j), pow(sigma_j(j),-2), mu_tj(t,j)*pow(sigma_j(j),2), true );
        }
        SIMULATE{
  ...
  // 4.2. Recruitment
  vector<Type> R_ln_sd(nspp); R_ln_sd.setZero(); // variance of init_devs FIXME set to beta_z(sp)
  matrix<Type> rec_dev(nspp, nyrs);              // Recruitment deviates
  ...
  array<Type>   non_par_sel(n_flt, max_sex, max_age, nyrs_hind); non_par_sel.setZero();// Estimated selectivity at age for AMAK non-parametric (pre-normalization)
  ...
  for(flt = 0; flt < n_flt; flt++) {
    ...
    case 2:  //  Non-parametric selectivity fit to age ranges (Ianelli 20?? & AMAK). NOTE: This can likely be improved
 ...
 */
  ...
  for(index_ind = 0; index_ind < index_obs.rows(); index_ind++) {
    ...
    if((flt_yr > 0) & (flt_yr <= endyr) & (flt_type(index) > 0)){
      if(index_obs(index_ind) > 0){
        ...
        // GOA Pollock 23
        jnll_comp(0, index) += 0.5*square((log(index_obs(index_ind, 0))-log(index_hat(index_ind))+square(index_std_dev)/2.)/index_std_dev);
      }
    }
  }
      ...
      if(((flt_varying_sel(flt) == 4)|(flt_varying_sel(flt) == 5)) & (flt_sel_type(flt) != 2) & (flt_sel_type(flt) != 5)){
        for(sex = 0; sex < nsex(sp); sex ++){
          for(yr = 1; yr < nyrs_hind; yr++){ // Start at second year
            ...
            // GOA pollock 2023
            jnll_comp(4, flt) += 0.5*square( (ln_sel_slp_dev(0, flt, sex, yr)-ln_sel_slp_dev(0, flt, sex, yr-1))/sel_dev_sd(flt));
            jnll_comp(4, flt) += 0.5*square( (sel_inf_dev(0, flt, sex, yr)-sel_inf_dev(0, flt, sex, yr-1))/(4*sel_dev_sd(flt)));
            ...
            if((flt_sel_type(flt) == 3) & (flt_varying_sel(flt) == 4)){
              ...
              //jnll_comp(4, flt) -= dnorm(ln_sel_slp_dev(1, flt, sex, yr) - ln_sel_slp_dev(1, flt, sex, yr-1), Type(0.0), sel_dev_sd(flt), true); // Pollock - remove "sd *4"

              // GOA pollock 2023
              jnll_comp(4, flt) += 0.5*square( (ln_sel_slp_dev(1, flt, sex, yr)-ln_sel_slp_dev(1, flt, sex, yr-1))/sel_dev_sd(flt));
              jnll_comp(4, flt) += 0.5*square( (sel_inf_dev(1, flt, sex, yr)-sel_inf_dev(1, flt, sex, yr-1))/(sel_dev_sd(flt)));// Pollock - remove "sd *4"
            }
  ...
  // GOA pollock 2023
  jnll_comp(12, 5) += -dnorm(ln_sel_slp(1,5,0), Type(0.0),Type(2.0), true);
  jnll_comp(12, 5) += -dnorm(sel_inf(1, 5, 0), Type(10.0),Type(3.0), true);
  ...
  for(flt = 0; flt < n_flt; flt++){
    ...
    if( est_index_q(flt) == 2){
      ...
      // GOA pollock 2023
      jnll_comp(5, flt) = 0.5*square((index_ln_q(flt)-index_ln_q_prior(flt))/index_q_sd(flt));
    }
    ...
    if((index_varying_q(flt) == 4) & (flt_type(flt) == 2) & ((est_index_q(flt) == 1) | (est_index_q(flt) == 2))){
      for(yr = 1; yr < nyrs_hind; yr++){
        //GOA pollock 2023
        jnll_comp(6, flt) += 0.5*square( (index_q_dev(flt, yr)-index_q_dev(flt, yr-1))/index_q_dev_sd(flt));
  ...
  vector<Type> sigmaR=exp(R_ln_sd);
  ADREPORT(sigmaR);
  REPORT( jnll );
  return jnll;
}
