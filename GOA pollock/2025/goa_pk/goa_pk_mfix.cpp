...
// ---------------------------------------------------------------------------
// GRANT: local copy of the 2024 final goa_pk source (23d) with the three
// source-side corrections used to reconcile goa_pk against Rceattle. Every
// change is tagged "GRANT" so a diff against reference/goa_pk_2024.cpp shows
// exactly what moved. See "2025/03-model.R" (RECONCILIATION LOG A1-A6).
//
//   A1. Initial age-structure M-index bug fix. initN(j) = initN(j-1)*exp(-M(j+1))
//       skips age-2 M and applies each age's FUTURE M; corrected to exp(-M(j)).
//   A3. Removed the q1/q2 random-walk penalties. q1 uses the Ecov link and q2
//       is fixed, so both deviate vectors are pinned at 0 and the penalties are
//       pure dnorm(0,0,rwlk_sd) normalizing constants (~527 units) that do not
//       affect the fit. Rceattle does not define q1/q2 as random walks.
//   A4. Added the lognormal bias correction (-sd^2/2) to the total catch, which
//       goa_pk already applies to the survey indices but not to catch.
//
// A2 (constant q3 random-walk SD), A5 (aging-error rows normalized) and A6
// (composition observations normalized) are data-side and are applied in R;
// see "2025/00-fit-goa_pk.R".
// ---------------------------------------------------------------------------
...
Type objective_function<Type>::operator() ()
{
  ...
  for (j=1;j<=8;j++) {
    ...
    // GRANT (A1): was exp(-M(j+1)), which skips age-2 M and applies each age's
    // FUTURE M to the survival step. The survival from initN(j-1) to initN(j)
    // is governed by M at the age initN(j-1) represents, i.e. M(j).
  ...
  for (i=0; i<nyears_proj; i++) {
    ...
    // Fishery likelihoods
    // Total catch
    // GRANT (A4): add the lognormal bias correction (-sd^2/2) to the expected
    // catch. goa_pk already does this for every survey index (see Eindxsurv*_tmp
    // below) but not for total catch; Rceattle bias-corrects both.
    vector<Type> Ecattot_tmp = log(Ecattot) - cattot_log_sd*cattot_log_sd/2.;
    loglik(0) = dnorm(log(cattot), Ecattot_tmp, cattot_log_sd, true).sum();
    SIMULATE {
      cattot=exp(rnorm(Ecattot_tmp, cattot_log_sd));
      REPORT(cattot);
    }
    ...
    for(i=y0+1;i<=y1;i++){
      ...
      // does not treat q1/q2 as random walks, so it never accumulates them.
