...
ms_mod_1 <- Rceattle::fit_mod(data_list = mydata_list[[1]],
...
plot_biomass(ms_mod_list[[1]])

mydata_2 <- mydata_list[[1]]
mydata_2$M1_base[1,3:32] <- 0.1 
mydata_2$M1_base[1,3] <- 0.1 
mydata_2$M1_base[2,3:32] <- 0.1
mydata_2$M1_base[3,3:32] <- 0.1
mydata_2$M1_base[4,3:32] <- 0.1
mydata_2$BTempC <- mydata_2$BTempC * 0 + 5.55042

ms_mod_2 <- Rceattle::fit_mod(data_list = mydata_2,
                                      inits = ss_run_base$estimated_params, # Initial parameters = 0
                                      file = NULL, # Don't save
                                      debug = 0, # Estimate
                                      random_rec = FALSE, # No random recruitment
...
plot_biomass(list(ss_run_base, ms_mod_2), model_names = c("SS", "MS"))

# Change M1 for all
ms_mod_list <- list()
for(i in 1:length(mydata_list)){
  mydata_list[[i]]$M1_base <- mydata_2$M1_base
