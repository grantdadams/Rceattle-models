...
################################################
# Read the data in
mydata_aaf <- Rceattle::read_data( file = "GOA_18.3.1_small_pcod_removed_coastwide_halibut_total_diet2.xlsx")
mydata_coastwide <- Rceattle::read_data( file = "GOA_18.3.1_small_pcod_removed_coastwide_halibut_total_diet2_bad_wt.xlsx")

# Note: diet data is from age 0-1, 1-2, rather than 1-1.5, 1.5-2.5, etc. Plus groups for diet data for ATF is 16 and for Halibut 16 as well.
...
for(i in 1:length(mydata_list)){
  mydata_list[[i]]$M1_base[1,3] <- 1.14 + 0.06169283
  mydata_list[[i]]$M1_base[1,4:6] <- c(0.5, 0.4, 0.34)
  mydata_list[[i]]$M1_base[2,3] <- 0.36
...
for(i in 1:length(mydata_list)){
  ms_mod_list[[i]] <- suppressWarnings(Rceattle::fit_mod(data_list = mydata_list[[i]],
                                   inits = ss_run_base$estimated_params, # Initial parameters = 0
                                   file = NULL, # Don't save
                                   debug = 0, # Estimate
                                   random_rec = FALSE, # No random recruitment
                                   msmMode = 1, # Single species mode
                                   silent = FALSE, phase = NULL,
                                   niter = 10))
}
