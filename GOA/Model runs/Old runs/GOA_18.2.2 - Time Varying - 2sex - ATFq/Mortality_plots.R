...
plot(y = ms_zed, x =  yrs, type = "l", lwd = 2, xlab = NA, ylab = NA, ylim = c(1.35, 1.55))
...
par(mfrow = c(3,1), mar = c(2.2,4.5,0.2,0.5))
# Pollock
ms_zed <- (ms_run_mod1$quantities$B_eaten[1,1,1,1:nyrs])/1000000
...
# Cod
ms_zed <- (ms_run_mod1$quantities$B_eaten[2,1,1,1:nyrs])/1000000
plot(y = ms_zed, x =  yrs, type = "l", lwd = 2, xlab = NA, ylab = NA, ylim = c(0, max(c(ms_zed))), xaxt = "n")
abline(h =  mean(ms_zed), lty = 2, col = "grey70", lwd = 2)
...
# ATF
ms_zed <- (ms_run_mod1$quantities$B_eaten[3,1,1,1:nyrs] + ms_run_mod1$quantities$B_eaten[3,2,1,1:nyrs])/1000000
