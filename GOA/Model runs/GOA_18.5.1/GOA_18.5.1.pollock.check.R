...
pollock_base$quantities$biomass[1,1:49] <- colSums(pollock_base$quantities$biomassByage[1,3:10,1:49]) * 2
pollock_rw$quantities$biomass[1,1:49] <- colSums(pollock_rw$quantities$biomassByage[1,3:10,1:49]) * 2
pollock_fix_q$quantities$biomass[1,1:49] <- colSums(pollock_fix_q$quantities$biomassByage[1,3:10,1:49]) * 2

######################### Plots
# - SAFE vs SS
file_name <- "Figures/18.5.1/18.5.1_SAFE_vs_ceattle_pollock"
mod_list <- list(pollock_base, pollock_rw, pollock_fix_q, pollock_safe)
mod_names <- c("CEATTLE pollock", "CEATTLE pollock rw", "CEATTLE fix BT q", "2018 SAFE (mt)")
for(i in 1:3){mod_list[[i]]$quantities$biomassSSB = mod_list[[i]]$quantities$biomassSSB * 2}
