...
#'
#' @examples
run_mse <- function(system = "GOA1977", om_list = NULL, om_names = NULL, em_hcr_list = NULL, em_hcr_names = NULL, sampling_period = NULL, nsim = 200, start_sim = 1, cap = NULL){
  ...
  for(om in 1:length(om_list)){  # OM model
    for(em in 1:length(em_hcr_list)){ # EM and HCR
      ...
      mse <- mse_run_parallel(om = om_list[[om]], 
                              ...
                              rec_trend = 0,
                              cap = cap, 
                              dir = paste0("Runs/", system,"/", om_names[om],"/", em_hcr_names[em]), 
                              file = NULL,
