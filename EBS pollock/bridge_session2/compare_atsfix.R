...
d$emp_sel <- es
# emp_sel (Selectivity=0) IGNORES Bin_first_selected (selectivity.hpp:324 skips
# type-0 fleets before normalize_and_project). Emulate the age-1 zeroing by hand
# (ccol[1] = the first comp column) so comp_hat excludes ATS/AVO age-1, as
# NonParametricPM + Bin_first_selected=2 would in the real match2.R config.
d$emp_sel[d$emp_sel$Fleet_name %in% c("ATS","AVO"), ccol[1]] <- 0
cat("zeroed ATS/AVO emp_sel age-1 (col", ccol[1], ")
")
