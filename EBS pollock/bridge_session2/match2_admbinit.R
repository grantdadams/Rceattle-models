...
# TEST: initialize the fishery selectivity at ADMB's coefficients, then estimate
# FREELY. If it stays -> the objective is multimodal (same likelihood, two optima);
# if it moves back to Rceattle's solution -> a real likelihood difference exists.
inits <- build_params(est)
inits$rec_pars[1,1] <- 9.601446   # ADMB log_avgrec (start near ADMB)
{
  ...
  coffs <- gp("sel_coffs_fsh")                                   # 12 base coffs (styr)
  devs  <- matrix(gp("sel_devs_fsh"), ncol=n_selages_fsh, byrow=TRUE)  # 60 change-yr x 12 INCREMENTS
  ych   <- 1965:2024
  nyr <- length(yrs)
  # TYPE 9 (NonParametricPM) is a carry-forward walk: np_unc(yr)=np_unc(yr-1)+sel_coff_dev(yr),
  # so sel_coff_dev is the per-year INCREMENT (0 at non-change years), NOT cumulative.
  # sel_coff = base coffs at styr. ADMB applies dev(ii) at change year i to log_sel(i+1),
  # so increment at Rceattle year (i+1) = sel_devs(ii) with yrs_ch_fsh(ii)=i.
  inits$sel_coff[1,1,1:n_selages_fsh] <- coffs
  inits$sel_coff_dev[1,1,1:n_selages_fsh,] <- 0
  for (k in seq_along(ych)) {
    yr_idx <- which(yrs == ych[k]) + 1L    # dev applied to the FOLLOWING year
    if (!is.na(yr_idx) && yr_idx <= nyr) inits$sel_coff_dev[1,1,1:n_selages_fsh, yr_idx] <- devs[k,]
  }
  # also init rec_dev / init_dev / log_F at ADMB
  ...
  cat("TEST: fishery sel + rec/F/init initialized at ADMB values\n")
}

cat(sprintf("=== match2_admbinit (fishery init at ADMB, estimated) vs m23_rceattle_full ===\n"))
fit <- try(Rceattle::fit_mod(data_list=est, inits=inits, file=NULL, estimateMode=0,
...
saveRDS(fit, file.path(SP, sprintf("match2_admbinit_bap%g.rds", BAP)))
...
cat("\n=== fishery sel 1964 age1..4 (converged): ", round(as.numeric(q$sel_at_age[1,1,1:4,1]),4), "===\n")
cat("(ADMB init was 0.024/0.167/0.564/1.366; Rceattle free optimum is 0.141/0.340/0.669/1.210)\n")
