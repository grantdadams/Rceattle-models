...
d$fleet_control$Bin_first_selected[fcn %in% c("ATS","AVO")] <- 2
inits<-build_params(d)
inits$rec_pars[1,1]<-get_par("log_avgrec"); inits$rec_dev[1,1:nyr]<-get_par("log_rec_devs")
inits$log_F[1,1:nyr]<-get_par("log_avg_F")+get_par("log_F_devs")
idv<-get_par("log_initdevs"); inits$init_dev[1,1:length(idv)]<-idv
...
readPA<-function(hdr){ i<-grep(hdr,orl)[1]; rows<-list(); j<-i+1
  while(j<=length(orl)){ v<-suppressWarnings(as.numeric(strsplit(trimws(orl[j]),"[[:space:]]+")[[1]]))
    if(length(v)<15||any(is.na(v[1:15]))) break; rows[[length(rows)+1]]<-v; j<-j+1 }
  m<-do.call(rbind,rows); rownames(m)<-m[,1]; m[,-1,drop=FALSE] }
ao<-readPA("Hydro Survey Observed P at age"); ah<-readPA("Hydro Survey Predicted P at age")
cat("ADMB Hydro obs dim:", dim(ao), " (ncol should be nages or nages-1)\n")
aidx<-which(cd2$Fleet_name=="ATS")
cat("=== ATS obs & pred P-at-age (Rce vs ADMB) ===\n")
for (yr in c(1994, 2010, 2024)) {
  ri<-aidx[cd2$Year[aidx]==yr]; if(length(ri)!=1){cat(yr,"skip\n");next}
  ro_obs<-co[ri,1:nages]; ro_hat<-ch[ri,1:nages]
  arow<-as.character(yr)
  if(!(arow %in% rownames(ao))){cat(yr,"not in ADMB\n");next}
  aoR<-ao[arow,]; ahR<-ah[arow,]
  cat(sprintf("\n--- %d --- sums: Rce_obs=%.3f Rce_hat=%.3f ADMB_obs=%.3f ADMB_hat=%.3f | ADMB ncol=%d\n",
      yr,sum(ro_obs),sum(ro_hat),sum(aoR),sum(ahR),length(aoR)))
  cat("age    Rce_obs   ADMB_obs   |   Rce_hat  ADMB_hat\n")
  for(a in 1:8) cat(sprintf(" %2d  %9.5f %9.5f   | %9.5f %9.5f\n",a,ro_obs[a],
      ifelse(a<=length(aoR),aoR[a],NA),ro_hat[a],ifelse(a<=length(ahR),ahR[a],NA)))
}
