...
N<-blk("N"); S<-blk("S"); Z<-blk("Z")
q<-fit$quantities
id <- fit$data_list$index_data; id$pred <- as.numeric(q$index_hat)
fc <- fit$data_list$fleet_control
a1code <- fc$Fleet_code[fc$Fleet_name=="ATS_1"]
iq <- q$index_q   # [flt, yr]
a1 <- which(id$Fleet_name=="ATS_1")
cat("=== ATS_1 decomposition (at ADMB MLE): does Rceattle ea1 = N_1*S^0.5? ===\n")
cat("Year     obs   Rce_pred    q_Rce   ea1_Rce=pred/q   N1*S^.5   ratio  N1*exp(-.5Z1)\n")
for (r in a1) {
  yr <- abs(id$Year[r]); k <- which(yrs==yr); yi <- k  # year index
  qv <- iq[a1code, k]
  ea1_rce <- id$pred[r]/qv
  ns <- N[k,1]*sqrt(S[k,1])
  nz <- N[k,1]*exp(-0.5*Z[k,1])
  cat(sprintf("%5d %8.1f %9.1f %8.4g %13.1f %9.1f %6.3f %11.1f\n",
      id$Year[r], id$Observation[r], id$pred[r], qv, ea1_rce, ns, ea1_rce/ns, nz))
}
cat("\nNote: ADMB ea1_ats = N_1 * S^0.5 = N_1*exp(-0.5*Z_1). S here is annual survival exp(-Z).\n")
