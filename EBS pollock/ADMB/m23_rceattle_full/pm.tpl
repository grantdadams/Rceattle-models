...
DATA_SECTION
 ...
 END_CALCS
  ...
  init_int phase_q_ats    // phase for estimating survey q for echo-integration trawl survey
	// Not sure why this is here...should be same for non-covariance runs too...
...
//  !! ad_comm::change_datafile_name(control_temp_pred_filename);
 ...
 LOCAL_CALCS    
  ...
  if (Use_78_on_only)
  {
    if (styr>1978) // to test if styr is before 1978
      styr_est = styr;
    else
      styr_est = 1978;
  }
 ...
 LOCAL_CALCS
  ...
  if (ad_comm::argc > 1)
  {
    ...
    if ( (on=option_match(argc,argv,"-Mmatrix"))>-1)
		{
      Mmatrix = 1;
		}
    ...
    if ( (on=option_match(ad_comm::argc,ad_comm::argv,"-condmsy"))>-1)
    {
      ...
      {
        cerr << "Invalid number of arguments for command line option -condmsy; option ignored" << endl;
      }
      else
      {
        condmsy = atoi(ad_comm::argv[on+1]);
        cout<<  "Currently using "<<adstring(ad_comm::argv[on+1])<<" as conditional msy"<<endl;
      }
    }
 ...
 LOCAL_CALCS
  // Flag to use covariance for bottom trawl survey, 0=use vector, 1=use cov matrix, 2=use GenGamma
   ...
   write_log(GenGamData); //exit(1);
	 if (Mmatrix){
     ad_comm::change_datafile_name("../data/Mmatrix.dat" );cout<<"Opening "<<Wtage_file<<endl; 
        *(ad_comm::global_datafile) >>  M_in; 
	   cout<<"Read in M-matrix"<<endl;
	 }
 END_CALCS
...
// read in data for doing temp-recuitment model, and spatial predation
 ...
 !! endyr_temp = 2023;
 init_vector SST(styr-1,endyr_temp-1);
 number SST_fut
 !! SST_fut = mean(SST(endyr_temp-7,endyr_temp-3));
 ...
 init_3darray poll_dist(1,n_pred_ages,styr,endyr_temp,1,nstrata_pred)  // the distribution of age 1 pollock across strata, by year
 init_3darray pred_dist_nonpoll(1,n_pred_grp,styr,endyr_temp,1,nstrata_pred)      // the distribution of predators by strata, by year 
 3darray  Npred_bystrata_nonpoll(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred)      // the number of non-pollock predators by strata for a given year
 ...
 init_matrix  temp_bystrata(styr,endyr_temp,1,nstrata_pred)  // the temperature by strata (year before 1982 are an average)
 init_matrix mn_wgt_nonpoll(1,n_pred_grp_nonpoll,styr,endyr_temp)                 // the mean weight of the predator groups
 ...
 3darray V_nonpoll(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred)                         // V matrix for max consumption temperature function 
 3darray F_t_nonpoll(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred)      
 3darray Cmax_nonpoll(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred)       // Cmax by year, predator group, and region
...
PARAMETER_SECTION
  ...
  // MODIFIED (m23_rceattle): log_avg_F is FIXED (phase -1, held at its
  // INITIALIZATION_SECTION value -1.6). With log_F_devs converted to a plain
  // vector (sum-to-zero removed) and the F penalty switched off
  // (control.dat ctrl_flag(4) = 0), an estimated log_avg_F would make the F level
  // perfectly confounded with the devs (n+1 parameters for n annual F's).
  // Fixing it leaves exactly one free parameter per year, matching Rceattle's
  // freely-estimated per-year log_F. The fixed offset is absorbed by the devs.
  ...
  // ---------------------------------------------------------------------------
  // MODIFIED (m23_rceattle_full): log_q_avo BOUNDED (was an unbounded init_number).
  // avo_like = sum((obs_avo - q_avo*X)^2 / (2*sigma^2)) is natural-scale normal with
  // an ABSOLUTE sigma, and q_avo = mfexp(log_q_avo). Its gradient
  //     d(avo_like)/d(log q) = -q * sum(X*(obs - q*X)/sigma^2)  ->  0   as q -> 0
  // vanishes as q -> 0, so q_avo -> 0 is a ZERO-GRADIENT TRAP: once it drifts small
  // the optimiser cannot climb back and reports "converged" while sitting on the
  // predict-nothing floor avo_like = sum(obs^2/(2 sigma^2)) = 296.185 (observed
  // exactly, with log_q_avo = -43.5 => q = 1e-19). Because q_avo affects ONLY
  // avo_like, the true optimum q* = sum(obs*X/s^2)/sum(X^2/s^2) is strictly positive
  // (~exp(-8)); the trap is purely a parameterisation artifact. Bounding keeps it in
  // a sane region. Reference m23 sits at log_q_avo = -8.18.
  // ---------------------------------------------------------------------------
  init_bounded_number log_q_avo(-15.,0.,phase_avo_q);
  ...
  // ---------------------------------------------------------------------------
  // MODIFIED (m23_rceattle): fishing mortality is FREELY estimated per year, as in
  // Rceattle (PARAMETER_MATRIX log_F; F = sel * exp(log_F(flt,yr)) -- no mean+dev
  // decomposition, no sum-to-zero, and NO F penalty). Two changes make ADMB match:
  //   (1) log_F_devs becomes a plain bounded vector (sum-to-zero removed);
  //   (2) log_avg_F is FIXED (phase < 0, below), so Fmort = exp(log_avg_F +
  //       log_F_devs) has exactly ONE free parameter per year, and log_F_devs is
  //       just the per-year log-F expressed as an offset from that fixed mean.
  // Both are needed together: with the sum-to-zero removed AND the F penalty off
  // (control.dat ctrl_flag(4) = 0), leaving log_avg_F estimated would give n+1
  // parameters for n annual F's -- the level would be perfectly confounded (add c
  // to log_avg_F, subtract c from every dev) and the Hessian singular.
  init_bounded_vector log_F_devs(styr,endyr_r,-15.,15.,2)
  //init_bounded_dev_vector log_F_devs(styr,endyr_r,-15.,15.,2)   // original m23 (sum-to-zero)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // MODIFIED (m23_rceattle): BTS selectivity deviations are declared as plain,
  // NON-BOUNDED vectors WITHOUT the dev_vector sum-to-zero constraint, to match
  // Rceattle's penalized random-walk deviations. Because removing sum-to-zero
  // leaves the overall level confounded with the base parameter, the FIRST year
  // is excluded from estimation (vectors start at styr_bts+1) and held at 0 in
  // the full-range vectors built in the PROCEDURE_SECTION. That is exactly what
  // Rceattle's build_map() does for LogisticPM + RandomWalk (first dev fixed).
  // The *_dev vectors below keep their original names/ranges so all downstream
  // code (compute_selectivity, the age-1 loop, the RW penalties) is unchanged.
  init_vector sel_slp_bts_dev_est(styr_bts+1,endyr_r,phase_logist_bts_dev+1)  // survey selectivity slope devs (yr 1 fixed at 0)
  init_vector sel_a50_bts_dev_est(styr_bts+1,endyr_r,phase_logist_bts_dev)    // survey selectivity inflection devs (yr 1 fixed at 0)
  init_vector sel_age_one_bts_dev_est(styr_bts+1,endyr_r,phase_age1devs_bts)  // survey selectivity age-1 devs (yr 1 fixed at 0)
  vector sel_slp_bts_dev(styr_bts,endyr_r)
 ...
 // Average weight stuff
  ...
  sdreport_matrix future_R(1,nscen,styr_fut,endyr_fut)
...
PROCEDURE_SECTION
  ...
  // MODIFIED (m23_rceattle): build the full-range BTS deviation vectors with the
  // first year held at 0 (see PARAMETER_SECTION). initialize() zeros the whole
  // vector; only the estimated years (styr_bts+1..endyr_r) are copied in, so
  // year styr_bts has dev = 0 and pins the level that the removed sum-to-zero
  // constraint used to pin.
  sel_slp_bts_dev.initialize();
  sel_a50_bts_dev.initialize();
  sel_age_one_bts_dev.initialize();
  sel_slp_bts_dev(styr_bts+1,endyr_r)     = sel_slp_bts_dev_est;
  sel_a50_bts_dev(styr_bts+1,endyr_r)     = sel_a50_bts_dev_est;
  sel_age_one_bts_dev(styr_bts+1,endyr_r) = sel_age_one_bts_dev_est;

  // MODIFIED (m23_rceattle): always compute wt_pre/wt_hat so wt_fut (used by the
  // MSY/F40 calculations every iteration) stays valid when the weight-at-age
  // submodel is switched OFF (phase_coheff/phase_yreff < 0). With those phases
  // negative the weight parameters are fixed at their INITIALIZATION_SECTION
  // values, so this is deterministic; wt_like is then a constant and is excluded
  // from the objective (see Evaluate_Objective_Function).
...
FUNCTION GetDependentVar 
  ...
  if (sd_phase()||mceval_phase())
  {
    ...
    {
      res.initialize(); 
      wt_fut(3,nages) = wt_pre(endyr_wt-iyr+1);
      if (!mceval_phase()) res = get_msy_wt();
...
FUNCTION Future_projections_fixed_F
  ...
  for (int k=1;k<=nscen;k++) {
    ...
    future_R(k,styr_fut)       = natage_future(k,styr_fut,1);
    ...
    // Now compute the time of spawning SSB for everything else....
      ...
      future_R(k,i)         = natage_future(k,i,1);
      if (i<endyr_fut)
      {
...
FUNCTION Get_Catch_at_Age
 ...
 //avo predicted values..
    for (i=1;i<=n_avo_r;i++)
    {
      ...
      // MODIFIED (m23_rceattle_full): pred_avo sums ages mina_ats..nages, EXCLUDING
      // age-1, to MATCH the age-2+ eb_ats (both borrow log_sel_ats, whose age-1 value
      // is only a normalisation residue, not a modelled selectivity). This makes the
      // mirrored ATS+AVO fleets share the SAME age range (2..15), which Rceattle
      // represents with both at Bin_first_selected = 2 -- no package change needed.
      // Prediction-side only; obs_avo (total acoustic biomass, incl. age-1) is left
      // untouched, and q_avo (now bounded) absorbs the ~1.4-3.9% constant offset.
      {
        dvariable avo_tmp = 0.0;
        for (int a=mina_ats;a<=nages;a++)
          avo_tmp += wt_avo(i,a) * natage(iyr,a) * mfexp(log_sel_ats(iyr,a));
        pred_avo(i) = avo_tmp * q_avo;
      }
 ...
 //Hydro survey expected values------------------------
  ...
  for (i=1;i<=n_ats_ac_r;i++)
  {
    ...
    // -------------------------------------------------------------------------
    // MODIFIED (m23_rceattle_full): eb_ats sums ages mina_ats..nages, EXCLUDING age-1,
    // so ATS age-1 is fitted ONLY by the dedicated age-1 index (ea1_ats/oa1_ats,
    // surv_like(3)) and no longer ALSO inside surv_like(2) -- i.e. no double-count.
    // The OBSERVATION ob_ats is deliberately left untouched: we do not know whether
    // the survey's reported total biomass excludes age-1 or not, so we do not edit
    // the data. q_ats absorbs the resulting constant offset (age-1 is 1.1-3.4% of
    // eb_ats); only the year-to-year variation in that share is unabsorbed.
    // sel_ats(1) is in any case not a modelled quantity: compute_selectivity_ats*
    // assigns coffs over (mina_ats..nsel) and (nsel+1..nages) only, so log_sel(*,1)
    // keeps its initialize() zero and emerges as 1/mean(exp(log_sel)) -- a pure
    // normalisation residue (0.767 -> 0.514 over time). Age-1 does not belong in a
    // biomass prediction weighted by a selectivity nobody estimated.
    // pred_avo is left alone: AVO's likelihood is natural-scale normal with an
    // absolute SD and no lognormal cushion, so it responds to a shape change by
    // driving q_avo -> 0 (see handoff 3e).
    // -------------------------------------------------------------------------
    {
      dvar_vector eac_tmp_ats(mina_ats,nages);
      dvector     wt_tmp_ats(mina_ats,nages);
      for (int a=mina_ats;a<=nages;a++) { eac_tmp_ats(a) = eac_ats(i,a); wt_tmp_ats(a) = wt_ats(i,a); }
      eb_ats(i) = wt_tmp_ats * eac_tmp_ats;
    }
...
FUNCTION Recruitment_Likelihood
 ...
 // ---------------------------------------------------------------------------
 // MODIFIED (m23_rceattle_full): FULL normal log-likelihoods, matching Rceattle.
 // Rceattle uses  -dnorm(dev, 0, R_sd, true)  =  dev^2/(2 sigma^2) + log(sigma)
 //                                              + 0.5*log(2*pi)   per dev,
 // with ONE sigma (sigma_rec_prior) shared by rec_dev and init_dev, applied
 // UNIFORMLY over every hindcast year. The stock forms here were kernels only
 // (1.*norm2), i.e. sigma = 1/sqrt(2) = 0.7071 with no sigma or 2*pi constants,
 // which makes tot_like incomparable to Rceattle's jnll. With sigr = 1 these
 // become exactly Rceattle's penalty at sigma_rec_prior = 1, bias_adjust_proc = 0.
 // ---------------------------------------------------------------------------
 ...
 // Regularizing penalty on all initial age-comp -devs (same full normal form)
  rec_like(4) = norm2(log_initdevs)/(2.*sigmaRsq)
              + double(size_count(log_initdevs))*log(sigr)
              + double(size_count(log_initdevs))*0.5*log(2.*M_PI);

  // ---------------------------------------------------------------------------
  // MODIFIED (m23_rceattle_full): rec_like(1) is forced to ZERO unconditionally,
  // whichever branch below runs. Rceattle applies exactly ONE recruitment-deviation
  // penalty (rec_like(2)); this second one has no Rceattle counterpart. Guarding it
  // here (rather than only inside the phase_sr<0 branch) means the alignment holds
  // regardless of phase_sr / phase_steepness settings.
  // ---------------------------------------------------------------------------
  int rec_like1_off = 1;   // m23_rceattle_full: keep rec_like(1) disabled
  ...
  if (phase_sr<0) 
  {
    ...
    // MODIFIED (m23_rceattle_full): rec_like(1) DISABLED. Rceattle applies exactly
    // ONE recruitment-deviation penalty (rec_like(2) above). The stock model added
    // a SECOND, windowed penalty over styr_est..endyr_est (1978-2022) -- and under
    // SrType = 3 (SRecruit == mfexp(log_avgrec), a constant) it was never a real
    // stock-recruit fit at all: SR_resids == log_rec_devs exactly (verified,
    // max|diff| = 4.9e-06). Combined with rec_like(2) it gave 1.5x^2+0.5x on
    // 1978-2022 excluding 1979 (ctrl_flag(25)>=1 drops the 1978 year-class) and
    // 1.0x^2 elsewhere -- a per-year-varying sigma that Rceattle's single
    // sigma_rec_prior cannot represent. Set phase_sr = -1 in control.dat to reach
    // this branch (which also deactivates the inert `steepness` parameter).
    rec_like(1) = 0.0;
  ...
  // MODIFIED (m23_rceattle_full): enforce rec_like(1) = 0 after either branch.
  if (rec_like1_off) rec_like(1) = 0.0;
 ...
 // This sets variability of future recruitment to same as in past....
  ...
  // WILL BREAK ON RETROSPECTIVE, and do we need this? if so should be rewritten 
  //if (ctrl_flag(29) > 0)
    //rec_like(5) += 10.*norm2(log_rec_devs(endyr_est,endyr_r))/(2.*sigmarsq_out+.001);
...
FUNCTION Evaluate_Objective_Function 
  ...
  // Prior on log_Rzero          
   ...
   if (mcmcmode  & !do_srrdevs) 
			Priors(4) = 6.6*square( log_Rzero - 10.10 ); // Set to value and std in MLE for stabilizing MCMC
...
FUNCTION Selectivity_Likelihood
  ...
  if (active(sel_a50_bts_dev_est))
  {
    ...
    if (active(sel_age_one_bts_dev_est))
...
FUNCTION Surv_Likelihood
  ...
  */
  ...
  if (use_age1_ats) 
  {
    ...
    // MODIFIED (m23_rceattle_full): when ignore_last_ats_age1, compute qtmp over the
    // SAME range as the likelihood (1..n_ats_r-1), i.e. EXCLUDE the dropped last year
    // from the analytical q as well. The stock form used all years for qtmp but
    // dropped the last from surv_like(3) -- an inconsistency. Excluding it from both
    // matches Rceattle's negative-year convention (a year<0 obs is excluded from the
    // likelihood AND the analytical q; ceattle_v01_11.cpp:1821 & :2454).
    dvariable qtmp;
    if (ignore_last_ats_age1)
    {
      qtmp = mfexp(mean(log(oa1_ats(1,n_ats_r-1))-log(ea1_ats(1,n_ats_r-1))));
      surv_like(3) = 0.5*norm2(log(oa1_ats(1,n_ats_r-1)+.01)-log(ea1_ats(1,n_ats_r-1)*qtmp +.01))/(age1_sigma_ats*age1_sigma_ats) ; 
    }
    else
    {
      qtmp = mfexp(mean(log(oa1_ats)-log(ea1_ats)));
...
FUNCTION SimulateData1
  ...
  // not in wt-age
    if (mceval_phase()) {
			// std::string simnotmp = (std::to_string(count_mcsave));
      // simname = "sim_"+ NumLbl(isim) + ".dat";
    //   simname = "em/pm_data_+NumLbl(iproj_yr)+".dat";
		}
    else {
      // simname = "sim_"+ adstring(std::to_string(isim)) + ".dat";
			// simno = adstring(isim);
      // simname = "sim_"+ NumLbl(isim) + ".dat";
      simname = "em/pm_data.dat";
			// std::string simnotmp = (std::to_string(isim));
      // simname = "sim_"+ adstring(simnotmp) + ".dat";
		}
  ...
  for (int isim=1;isim<=nsims;isim++)
  {
		...
		dmatrix oac_fsh_sim(1, nyrs_future, 1, nages);
		dmatrix oac_bts_sim(1, nyrs_future, 1, nages);
		dmatrix oac_ats_sim(1, nyrs_future, 1, nages);
		dvector obs_catch_sim(1, nyrs_future);
		dvector obs_avo_sim(1, nyrs_future);
		dvector obs_ats_sim(1, nyrs_future);
		dvector obs_bts_sim(1, nyrs_future);
		dvector ot_ats_sim(1, nyrs_future);
		dvector ot_bts_sim(1, nyrs_future);
		ivector yrs_ats_sim(1, n_ats+nyrs_future);
   ...
   // 2010 survey timing N
    ...
    // EIT expectation for 2007 projection
    eac_ats_fut = value(elem_prod(ntmp,mfexp(log_sel_ats(endyr_r))) * q_ats); 
		for (int itmp=1;itmp<=n_ats;itmp++)
			yrs_ats_sim(itmp) = yrs_ats_data(itmp);
		for (int itmp=1;itmp<=nyrs_future;itmp++)
			yrs_ats_sim(itmp+n_ats) = yrs_ats_data(n_ats)+itmp;

    for (int iproj_yr=1;iproj_yr<=nyrs_future;iproj_yr++)
    {
       simname = "em/pm_data_" + NumLbl(iproj_yr) + ".dat" ;
       i=styr_fut+iproj_yr;
       natage_futsim(i)(2,nages)  = ++elem_prod(natage_futsim(styr_fut)(1,nages-1), S(endyr_r)(1,nages-1));  
       ...
       simwrite( wt_fsh           );
       // Simulate next year's wt-age from stochsastic history? xxx
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simwrite( wt_fut           );
       ...
       catchin >>catchtmp;
       catchin.close();
		   obs_catch_sim(iproj_yr) = catchtmp;
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simdat << obs_catch_sim(itmp)      <<endl;
       ...
       simwrite( n_avo+iproj_yr);
       simwrite( yrs_avo);
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simdat << yrs_avo(n_avo) + itmp <<endl;
      
       sigma   = 0.250;
       sigmasq = sigma*sigma;
       ran_age_vect.fill_randn(rng);
       dvector avo_tmp(1,nages);
       avo_tmp = value(mfexp(-sigmasq/2.) * elem_prod(mfexp(sigma * ran_age_vect) , eac_ats_fut));
		   obs_avo_sim(iproj_yr) = value(avo_tmp*wt_avo(n_avo)*q_avo  );
   
       simwrite( obs_avo);
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simdat << obs_avo_sim(itmp)<<" ";
		   simdat<<endl;
   
       simwrite( obs_avo_std);
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simdat << 0.25*obs_avo_sim(itmp)<<" ";
		   simdat<<endl;
   
       simwrite( wt_avo);
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simdat<< wt_avo(n_avo)<<endl;

       simwrite( ngears          );
       simwrite( minind          );
       // Add one year of fishery and survey years
       simwrite( n_fsh+iproj_yr  );
       simwrite( n_bts+iproj_yr  );
       int new_ats;
		   if ((endyr+iproj_yr) % 2 == 0) 
			   new_ats=1;
	     else
	  	   new_ats=0;
   
         // simdat << endyr+iproj_yr <<endl;// Nota advance to next year
		   if (new_ats)
         nats++;
		   simwrite(nats);
       ...
       simwrite( yrs_fsh_data    );
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
						   simdat << yrs_fsh_data(n_fsh) + itmp   <<endl;
     
       simwrite( yrs_bts_data             );
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
				   simdat << yrs_bts_data(n_bts) + itmp   <<endl;
     
       simwrite( yrs_ats_data             );
		   // if ((endyr+iproj_yr) % 2 == 0) yrs_ats_sim(iproj_yr+endyr) = yrs_ats_sim(iproj_yr+endyr-1) + 2;
   
       for (int itmp=n_ats+1;itmp<=n_ats+iproj_yr;itmp++) 
	     {
			   cout<<itmp<<" "<<yrs_ats_sim(itmp)<<endl;
		     if (yrs_ats_sim(itmp+1) % 2 == 0) 
			   {
				   cout<<itmp<<" "<<yrs_ats_sim(itmp+1)<<endl;
			     simdat << yrs_ats_sim(itmp)+1<<endl;
			   }
	     }
  
       simwrite( sam_fsh         );
       simrep(sam_fsh(n_fsh),iproj_yr);
       //simdat << sam_fsh(n_fsh)  <<endl;
     
       simwrite( sam_bts         );
       simrep(sam_bts(n_bts),iproj_yr);
       // simdat << sam_bts(n_bts)  <<endl;
       ...
       simwrite( oac_fsh);
		   oac_fsh_sim(iproj_yr) = value(exp(-sigmasq/2.) * elem_prod(mfexp(sigma * ran_age_vect) , eac_fsh_fut) );
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simwrite( oac_fsh_sim(itmp) ) ;
     
       // Simulate next year's bt survey age compositions and totals....
       sigma   = 0.150;
       sigmasq = sigma*sigma;
       dvector bts_tmp(1,nages);
       bts_tmp = value(mfexp(-sigmasq/2.) * elem_prod(mfexp(sigma * ran_age_vect) , eac_bts_fut));
       simwrite( obs_bts_data); 
       // bts biomass
		   obs_bts_sim(iproj_yr) = value(bts_tmp * wt_bts(n_bts)*q_bts); 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     simdat<< obs_bts_sim(itmp)<<" ";
		   simdat<<endl;
   
       // bts std
       simwrite(std_ob_bts_data) ; 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     simdat<< obs_bts_sim(itmp)*.2<<" ";
		   simdat<<endl;

       simwrite(wt_bts) ; 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     simdat << wt_bts(n_bts)             <<endl;

       // bts abundance
       simwrite(std_ot_bts); 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     simdat << " "<< sum(bts_tmp)*.15 <<endl;
       ...
       // bts age comp
       simwrite(oac_bts_data) ; 
		   oac_bts_sim(iproj_yr) =  bts_tmp;
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
         simdat<<(oac_bts_sim(itmp) )<<endl; ; 
       ...
       simwrite(oac_ats_data); 
         ...
         ran_age_vect.fill_randn(rng);
         ats_tmp = value(mfexp(-sigmasq/2.) * elem_prod(mfexp(sigma * ran_age_vect) , eac_ats_fut));
		     oac_ats_sim(iproj_yr) = ats_tmp;
		   }
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     if ((endyr+itmp) % 2 == 0) {
           simdat <<(oac_ats_sim(itmp)) <<endl;;
		     }

       simwrite(std_ot_ats); 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     if ((endyr+itmp) % 2 == 0) {
           simdat <<.2*sum(oac_ats_sim(itmp)) <<endl;;
		     }

       simwrite( obs_ats_data); 
		   if ((endyr+iproj_yr) % 2 == 0) 
				 obs_ats_sim(iproj_yr) = value(ats_tmp * wt_ats(n_ats)*q_ats );

       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     if ((endyr+itmp) % 2 == 0) 
	  	 simdat<<" "<<obs_ats_sim(itmp) ;
	     simdat<<endl;

       simwrite( std_ob_ats_data); 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     if ((endyr+itmp) % 2 == 0) 
	      	  simdat<<" "<<0.15*obs_ats_sim(itmp) ;
	     simdat<<endl;

       simwrite(   wt_ats ); 
       for (int itmp=1;itmp<=iproj_yr;itmp++) 
		     if ((endyr+itmp) % 2 == 0) 
		       simdat << wt_ats(n_ats)    <<endl;
	  	  // simdat<<" "<<0.15*obs_ats_sim(itmp) ;
	     simdat<<endl;
       simwrite( bottom_temp); simrep(" 4 " ,iproj_yr);
       simwrite(n_age_err);
       simwrite(age_err);
       simwrite( nlbins);
       simwrite( olc_fsh); 
 ...
 // get spr rates for ABC and OFL
  ...
  "Scen Catch  SSBNext  AdjNext  ABC1  OFL1  SSB2yrs  Adj2yrs  ABC2  OFL2 T2_ABC1 T2_OFL1 T2_ABC2 T2_OFL2 T1.5_ABC1 T1.5_ABC2"<<endl;
   for (i=1;i<=10;i++)
   {
	 ...
	 proj_df << "Scenario, Year, SSB, Catch, R, SER"<<endl;
	 for (int k=1;k<=nscen;k++) {
	   for (int i=styr_fut;i<=endyr_fut;i++) {
	     proj_df << 
			 k <<","<< 
...
FUNCTION Est_Fixed_Effects_wts
  ...
  // MODIFIED (m23_rceattle): only add the weight-at-age submodel likelihood to the
  // objective when it is actually being estimated. With phase_coheff/phase_yreff < 0
  // the weight parameters are fixed, so wt_like is a constant (zero gradient): it
  // cannot change the MLE, and excluding it keeps tot_like directly comparable with
  // Rceattle, which takes weight-at-age as fixed data (no weight submodel).
  if (active(coh_eff)||active(yr_eff))
    fff += wt_like;
  ...
  wt_yraf = wt_pre(endyr_wt); //*exp(sigma_coh*sigma_coh/2. + sigma_yr*sigma_yr/2.);;
...
GLOBALS_SECTION
  ...
  #undef simwrite
  #define simwrite(object) simdat << "#"<<#object  << endl << object << endl;

  #undef simrep
  #define simrep(object,n) for (int itmp=1;itmp<=n;itmp++) {simdat << object<<" "; } simdat<< endl;

  #undef COUT
  #define COUT(object) cout << #object "," << object << endl;

  ofstream proj_df("proj_df.csv");
  #undef proj_df
  #define proj_df(object) proj_df << #object "\n" << object << endl;
