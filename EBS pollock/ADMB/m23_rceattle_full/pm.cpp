...
p_mature *= 0.5;
 ...
 ycin=0;
  ...
  SSB_1.allocate(1,10);
  SSB_2.allocate(1,10);
  M_in.allocate(styr,endyr,1,nages);
  do_fmort=0;
  do_nodata=0;
  do_check=0;  
  Mmatrix=0;  
...
long int lseed=iseed;
 ...
 cout <<nstrata<<endl;
  ...
  nobs.allocate(1982,2023,1,14);
  mnCPUE.allocate(1982,2023,1,14);
  mntemp.allocate(1982,2023,1,14);
  temp_in.allocate(1982,2023,1,14,1,190);
  CPUE_in.allocate(1982,2023,1,14,1,190);
  write_log(sqkm);
  write_log(ndata);
  write_log(d);
  nobs.initialize();
  ...
  temp.allocate(1982,2023,1,14,1,nobs);
  CPUE.allocate(1982,2023,1,14,1,nobs);
  temp.initialize();
  CPUE.initialize();
  mntemp.initialize();
  mnCPUE.initialize();
 ...
 write_log(Temp_Cons_Dist_file);
 endyr_temp = 2023;
  SST.allocate(styr-1,endyr_temp-1,"SST");
 ...
 n_pred_grp = n_pred_grp_nonpoll + n_pred_grp_poll;         // the number of predator groups
  N_pred.allocate(1,n_pred_grp,styr,endyr_temp,"N_pred");
  ...
  poll_dist.allocate(1,n_pred_ages,styr,endyr_temp,1,nstrata_pred,"poll_dist");
  pred_dist_nonpoll.allocate(1,n_pred_grp,styr,endyr_temp,1,nstrata_pred,"pred_dist_nonpoll");
  Npred_bystrata_nonpoll.allocate(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred);
  ...
  temp_bystrata.allocate(styr,endyr_temp,1,nstrata_pred,"temp_bystrata");
  mn_wgt_nonpoll.allocate(1,n_pred_grp_nonpoll,styr,endyr_temp,"mn_wgt_nonpoll");
  Y_nonpoll.allocate(1,n_pred_grp_nonpoll);
  Z_nonpoll.allocate(1,n_pred_grp_nonpoll);
  X_nonpoll.allocate(1,n_pred_grp_nonpoll);
  V_nonpoll.allocate(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred);
  F_t_nonpoll.allocate(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred);
  Cmax_nonpoll.allocate(styr,endyr_temp,1,n_pred_grp_nonpoll,1,nstrata_pred);
...
{
  L1.set_initial_value(28.3062069824);
  L2.set_initial_value(44.8853389604);
  log_alpha.set_initial_value(-11.0000000000);
  log_K.set_initial_value(-0.135566578465);
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 cout <<phase_bt<<" Phase for bts temperature"<<endl;
  ...
  log_q_avo.allocate(-15.,0.,phase_avo_q,"log_q_avo");
  ...
  sigr.allocate(0.1,20.,phase_sigr,"sigr");
  ...
  sel_slp_bts_dev_est.allocate(styr_bts+1,endyr_r,phase_logist_bts_dev+1,"sel_slp_bts_dev_est");
  sel_a50_bts_dev_est.allocate(styr_bts+1,endyr_r,phase_logist_bts_dev,"sel_a50_bts_dev_est");
  sel_age_one_bts_dev_est.allocate(styr_bts+1,endyr_r,phase_age1devs_bts,"sel_age_one_bts_dev_est");
  sel_slp_bts_dev.allocate(styr_bts,endyr_r,"sel_slp_bts_dev");
  #ifndef NO_AD_INITIALIZE
    sel_slp_bts_dev.initialize();
  #endif
  sel_a50_bts_dev.allocate(styr_bts,endyr_r,"sel_a50_bts_dev");
  #ifndef NO_AD_INITIALIZE
    sel_a50_bts_dev.initialize();
  #endif
  sel_age_one_bts_dev.allocate(styr_bts,endyr_r,"sel_age_one_bts_dev");
  #ifndef NO_AD_INITIALIZE
 ...
 MN_const = 1e-3; // Multinomial constant
  ...
  #endif
  L1.allocate(10,50,phase_coheff-1,"L1");
  L2.allocate(30,90,phase_coheff-1,"L2");
  log_alpha.allocate(-1,"log_alpha");
  log_K.allocate(phase_coheff,"log_K");
  ...
  Rmsy.allocate("Rmsy");
  Bmsy.allocate("Bmsy");
  Fcur_F35.allocate(1,nscen,"Fcur_F35");
  ...
  #endif
  future_R.allocate(1,nscen,styr_fut,endyr_fut,"future_R");
...
dvariable model_parameters::robust_p(const dmatrix& obs,const dvar_matrix& pred,const dvariable& a, const data_vector& b)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
void model_parameters::SimulateDataSets(void)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvar_matrix model_parameters::compute_selectivity_ats_devs(const int nsel,const int stsel, dvariable& avgsel,const dvar_vector& coffs,const dvar_matrix& sel_devs)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
...
dvariable model_parameters::dgengamma(const double& x, dvariable mean2, const double& sigma, const double& Q)
{
  ofstream& srecout= *pad_srecout;
  ofstream& projout= *pad_projout;
  ofstream& nofish= *pad_nofish;
