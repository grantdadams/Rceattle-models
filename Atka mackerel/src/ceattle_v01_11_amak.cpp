 ...
 */
  ...
  for(index_ind = 0; index_ind < index_obs.rows(); index_ind++) {
    ...
    if((flt_yr > 0) & (flt_yr <= endyr) & (flt_type(index) > 0)){
      if(index_obs(index_ind) > 0){
        ...
        // iyr = int(yrs_ind(k,i));
        jnll_comp(0, index) += square(log(index_obs(index_ind, 0)) - log(index_hat(index_ind)) ) /
          (2.0 * index_std_dev*index_std_dev);
      }
    }
  }
  ...
  for(fsh_ind = 0; fsh_ind < catch_obs.rows(); fsh_ind++) {
    ...
    if((flt_yr > 0) & (flt_yr <= endyr) & (flt_type(flt) == 1)){
      if(catch_obs(fsh_ind, 0) > 0){
        ...
        // Martin's
        // jnll_comp(1, flt)+= 0.5*square((log(catch_obs(fsh_ind, 0) +.0001)-log(catch_hat(fsh_ind) +.0001 ))/fsh_std_dev);
  ...
  for(comp_ind = 0; comp_ind < comp_obs.rows(); comp_ind++) {
    ...
    comp_obs_tmp += 0.001; // AMAK
    comp_hat_tmp += 0.001; // AMAK
        ...
        for(yr = 0; yr < nyrs_tmp; yr++){
          ...
          for(sex = 0; sex < nsex(sp); sex++){
            for(age = 0; age < (nages(sp) - 1); age++) {
              ...
              jnll_comp(3, flt) += sel_curve_pen(flt, 0) * square( (CppAD::abs(sel_ratio_tmp) + sel_ratio_tmp))/2.0; // FIXME convert pen to variance
            }
          }
          ...
          for(sex = 0; sex < nsex(sp); sex++){
            ...
            for(age = 0; age < nages(sp) - 2; age++) {
              ...
              jnll_comp(4, flt) += sel_curve_pen(flt, 1) * pow( sel_tmp(age) , 2); //FIXME: convert pen to variance
            }
          }
          ...
          if(yr > 0){
            for(sex = 0; sex < nsex(sp); sex++){
              for(age = 0; age < (nages(sp) - 1); age++) {
                ...
                // AMAK
                jnll_comp(6, flt) += 0.5 * square((log( non_par_sel(flt, sex, age, yr))-log( non_par_sel(flt, sex, age, yr - 1)))/sel_dev_sd(flt));
              }
            }
          }
          ...
          for(sex = 0; sex < nsex(sp); sex++){
            jnll_comp(13, flt) += 20.0 * square(avg_sel(flt, sex, yr));
          }
        }
      }
  ...
  for(flt = 0; flt < n_flt; flt++){
    ...
    if( est_index_q(flt) == 2){
      ...
      // AMAK
      jnll_comp(5, flt) += square((index_ln_q(flt) - index_ln_q_prior(flt)))/(2.0*index_q_sd(flt)*index_q_sd(flt));
    }
